...
namespace Seldon
{
  ...
  */
  ...
  {
    row.Reallocate(this->n_);
    row.Zero();
    for (int j = 0; j < this->val_(i).GetM(); j++)
      row(this->val_(i).Index(j)) = this->val_(i).Value(j);
  }
  ...
  {
    col.Reallocate(this->m_);
    col.Zero();
    for (int i0 = 0; i0 < this->m_; i0++)
      for (int j = 0; j < this->val_(i0).GetM(); j++)
        if (this->val_(i0).Index(j) == i)
          col(i0) = this->val_(i0).Value(j);      
  }
  ...
  SetDenseRow(int i, const Vector<T>& row)
  {
    ...
    int nb_interac = 0;
    for (int j = 0; j < row.GetM(); j++)
      if (row(j) != zero)
        nb_interac++;

    this->val_(i).Reallocate(nb_interac);
    nb_interac = 0;
    for (int j = 0; j < row.GetM(); j++)
      if (row(j) != zero)
        {
          this->val_(i).Index(nb_interac) = j;
          this->val_(i).Value(nb_interac) = row(j);
          nb_interac++;
        }
  }
    ...
    for (int i0 = 0; i0 < this->m_; i0++)
      {
        ...
        for (int j = 0; j < this->val_(i0).GetM(); j++)
          if (this->val_(i0).Index(j) == i)
            {
              col_found = true;
              this->val_(i0).Value(j) = col(i0);
            }

        if (!col_found && (col(i0) != zero))
          this->AddInteraction(i0, i, col(i0));
      }
  }
  ...
  */
  ...
  GetSparseRow(int i, Vector<T, VectSparse>& row) const
  {
    ...
    for (int i0 = 0; i0 < i; i0++)
      for (int j = 0; j < this->val_(i0).GetM(); j++)
        if (this->val_(i0).Index(j) == i)
          row.AddInteraction(i0, this->val_(i0).Value(j));
  }
    ...
    for (int i0 = 0; i0 < i; i0++)
      {
        ...
        for (int j = 0; j < this->val_(i0).GetM(); j++)
          if (this->val_(i0).Index(j) == i)
            {
              col_found = true;
              this->val_(i0).Value(j) = row(i0);
            }

        if (!col_found && (row(i0) != zero))
          this->AddInteraction(i0, i, row(i0));
       }

    // upper part
    int nb_interac = 0;
    for (int j = i; j < row.GetM(); j++)
      if (row(j) != zero)
        nb_interac++;
