...
namespace Seldon
{
  ...
  //! solves a linear system with Jacobi method
  template<class T, class Vector1>
  int SolveJacobi(const VirtualMatrix<T>& A, Vector1& x, const Vector1& b,
                  const Vector<T>& invDiagonal, 
    ...
    while (! iter.Finished(r))
      {
        ...
        // we compute x_{n+1} = D^{-1} r + x_n
        for (int i = 0; i < N; i++)
          x(i) += invDiagonal(i)*r(i);
        ...
        /*iter.Mlt(A, r, z);
        //alpha = DotProd(r, r) / DotProd(z, r); // pas optimal
        alpha = DotProd(z, r) / DotProd(z, z); // residu minimum
        Add(alpha, r, x);
        Add(-alpha, z, r);
        */
	++iter;
  ...
  //! solves a linear system with S.O.R method
  template<class T, class Vector1>
  int SolveSOR(const VirtualMatrix<T>& A, Vector1& x, const Vector1& b,
               const Real_wp& om, Iteration<typename ClassComplexType<T>::Treal>& iter,
               int stage_ssor)
  {
    const int N = A.GetM();
    if (N <= 0)
    ...
    while (! iter.Finished(r))
      {
        // one step of SOR
        A.ApplySor(SeldonNoTrans, x, b, om, 1, stage_ssor); 
