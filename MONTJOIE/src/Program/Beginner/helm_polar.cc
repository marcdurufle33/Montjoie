...
void RunAll(EllipticProblem<TypeEquation>& var, const string& input_file, const string& name_elt)
{
  ...
  while (test_loop)
    {
      ...
      var.PerformFactorizationStep(mat_lu, mat_lu_sp, pivot, l);
      ...
      if (rhs(l).GetM() > 0)
	{
	  var.ComputeSolution(mat_lu, mat_lu_sp, pivot, rhs(l));
	  x_inf = max(x_inf, rhs(l).GetNormInf());
	  //DISP(l); DISP(x_inf); rhs(l).Write("sol.dat");
	  nb_modes_all++;
	}
  ...
  cout << "Number of total modes = " << nb_modes_all << endl;
  ...
  if (argc > 1)
    {
      ...
      EllipticProblem<HelmholtzEquationPolar> vars;
