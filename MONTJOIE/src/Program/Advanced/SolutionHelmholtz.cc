...
//! class for solving Helmholtz equation in spherical layers
/*!
  On each spherical layer, the solution solves Helmholtz equation
  ...
  constant rho and mu) or dielectric spherical crown, and a Dirichlet,
  Neumann or impedance condition condition is set on the intern boundary r = r0
  
 */
class AnalyticalHelmholtzSphereSolver : public InputDataProblem_Base
{
public :
  ...
  //! if true, we replace sommerfeld condition by a first order condition placed on an external sphere
  ...
  //! radius of all the spherical layers
  Vector<Real_wp> ri_sphere;
...
public:
  AnalyticalHelmholtzSphereSolver();
  ...
  void ComputeSolution(const Vector<VectComplex_wp>& alpha,
                       ...
                       const VectR3& Points, VectComplex_wp& Usol);
...
//! default constructor
AnalyticalHelmholtzSphereSolver::AnalyticalHelmholtzSphereSolver()
{
  ...
  first_order_abc = false; radius_sphere_outside = 2.0;
...
void AnalyticalHelmholtzSphereSolver::SetInputData(const string& description_field, const VectString& parameters)
{
  if (!description_field.compare("TypeOutput"))
    {
      if (parameters.GetM() <= 1)
        {
          cout << "In SetInputData of AnalyticalHelmholtzSphereSolver" << endl;
...
void AnalyticalHelmholtzSphereSolver::ComputeSolution(const Vector<VectComplex_wp>& alpha,
                                                      const Vector<VectComplex_wp>& beta,
                                                      const VectR3& Points, VectComplex_wp& Usol)
{
  int N_max = n_modes;
  ...
  Vector<Real_wp> Pn_cos_teta(N_max);
  
  Usol.Reallocate(Points.GetM());
  Complex_wp u_loc, kr;
  Real_wp x, y, z, teta, phi, r, cos_teta, sin_teta;
  for (int i = 0; i < Points.GetM(); i++)
    {
	  ...
	  // Spherical Bessel functions and derivatives are computed
          ComputeDeriveSphericalBesselHankel(begin_order, N_max, kr, Jn, Hn, Jn_prime, Hn_prime);
          ...
          for (int n = 0; n < N_max; n++)
	    {
          ...
          Pn.EvaluatePn(N_max, cos_teta, Pn_cos_teta);
	      
          // computing the total field
          Complex_wp mI_exp_n; SetComplexOne(mI_exp_n);
          Complex_wp coef_n;
          if (type_data == TRACE_U)
            {
              if (num == ri_sphere.GetM()-2)
                for (int n = 0; n < N_max; n++)
                  {
                    coef_n = Real_wp(2*n+1)*mI_exp_n;
                    u_loc += (Jn(n) + alpha(n)(num)*Hn(n)+beta(n)(num)*Hn2(n))*coef_n*Pn_cos_teta(n);
                ...
                for (int n = 0; n < N_max; n++)
                  {
                    coef_n = Real_wp(2*n+1)*mI_exp_n;
                    u_loc += (alpha(n)(num)*Hn(n)+beta(n)(num)*Hn2(n))*coef_n*Pn_cos_teta(n);
          ...
          else if (type_data == TRACE_DU_DN)
            {
              if (num == ri_sphere.GetM()-2)
                for (int n = 0; n < N_max; n++)
                  {
                    coef_n = Real_wp(2*n+1)*mI_exp_n;
                    u_loc += (alpha(n)(num) * Hn_prime(n) + beta(n)(num)*Hn2_prime(n))*coef_n*Pn_cos_teta(n);
                    mI_exp_n *= -Iwp;
                  }
              else
                for (int n = 0; n < N_max; n++)
                  {
                    coef_n = Real_wp(2*n+1)*mI_exp_n;
                    u_loc += (Jn_prime(n) + alpha(n)(num) * Hn_prime(n) + beta(n)(num)*Hn2_prime(n))*coef_n*Pn_cos_teta(n);
                    mI_exp_n *= -Iwp;
                  }
            }          
          ...
          if (!total_field)
            u_loc -= exp(-Iwp*kwave(ri_sphere.GetM()-2)*z);
        }
...
void AnalyticalHelmholtzSphereSolver
...
void AnalyticalHelmholtzSphereSolver::WriteTimeSolution()
{
  // times t
...
::ComputeDecompositionCoef(Vector<VectComplex_wp>& alpha,
                           Vector<VectComplex_wp>& beta)
{
  if ((rho.GetM() != mu.GetM()) || (rho.GetM() != ri_sphere.GetM()-1))
    {
      cout << "There must be N-1 physical indices rho and mu where N is the number"
           << " of radii" << endl;
      
      cout << "Current radii are " << ri_sphere << endl;
  ...
  if (type_input_modes == MODES_PYTHON)
    {
      ...
      ifstream file_in("MATLAB/helmholtz_sphere.py");
      file_out.precision(15);
      
      if (!file_in.is_open())
	{
	  cout << "Unable to open file MATLAB/helmholtz_sphere.py" << endl;
	  abort();
	}
  ...
  for (int i = 0; i < N; i++)
    {
      ...
      // Spherical Bessel functions and derivatives are computed
      ComputeDeriveSphericalBesselHankel(begin_order, N_max, kr,
  ...
  for (int n = 0; n < N_max; n++)
    {
      ...
      // first radius -> if equal to 0, we have a dielectric internal ball
      if (ri_sphere(0) == Real_wp(0))
        {
      ...
      if (first_order_abc)
        {
          ...
          Real_wp Kappa = 2.0/ri_sphere(N);
          Real_wp n2 = n*(n+1)/square(ri_sphere(N)); Complex_wp k = kwave(N-1);
          ...
          if (grazing_abc)
            {
              Complex_wp coeff = pow(k*k, Real_wp(1)/3)*exp(-Iwp*pi_wp/3.0)*pow(6.0*Kappa,Real_wp(1)/3.0)
		*tgamma(2.0/3.0)/tgamma(1.0/3.0);
              if (order_abc == 0)
                {
                  beta_cla = coeff;
                }
...
int main(int argc, char** argv)
{
  ...
  AnalyticalHelmholtzSphereSolver vars;  
