...
int main(int argc, char **argv) 
{
  ...
  // on construit d'abord PyramidHdivOptimalHpFirstFamily
  //PyramidHdivOptimalHpFirstFamily test_elt;
  //PyramidHdivHpFirstFamily test_elt;
  PyramidHdivOther test_elt;
  test_elt.type_basis = test_elt.NIGAM_PHILLIPS_1;
  test_elt.ConstructFiniteElement(r);

  // on recupere une formule de quadrature d'ordre max(r, rf) + 1 pour la pyramide
  VectR3 points3d;
  VectReal_wp weights3d, points1d_z, weights1d_z;
  PyramidQuadrature::ConstructQuadrature(max(r, rf) + 1, points3d, weights3d, points1d_z, weights1d_z, 
                                         PyramidQuadrature::QUADRATURE_GAUSS);
  ...
  PyramidHdivFirstFamily first_elt;
  //PyramidHdivOther first_elt;
  //TetrahedronHdivFirstFamily first_elt;
  //first_elt.type_basis = test_elt.NIGAM_PHILLIPS_1;
  first_elt.ConstructOrthogonalBasis(rf);
  //PyramidHcurlHpFirstFamily first_elt;
  ...
  PyramidHdivOptimalFirstFamily opt_elt;
  opt_elt.ConstructOrthogonalBasis(rf);
