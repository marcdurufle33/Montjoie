...
namespace Montjoie
{
  ...
  class HexahedronHcurlFirst : public HexahedronReference<2>
  {
    public :
    ...
    HexahedronHcurlFirst() : HexahedronReference()
    {
    ...
    {
      HexahedronReference::ConstructFiniteElement(r+1);
              ...
              for (int j = 0; j <= r ; j++)
                {
                  if (i > 0)
                    {
                      P(ind)(0) = Pow(x,i-1)*Pow(y,j)*Pow(z,k);
                      P(ind)(1) = zero;
                      P(ind)(2) = zero;
                      ind++;
                    }
                  ...
                  if (j > 0)
                    {
                      P(ind)(0) = zero;
                      P(ind)(1) = Pow(x,i)*Pow(y,j-1)*Pow(z,k);
                      P(ind)(2) = zero;
                      ind++;
                    }
                  ...
                  if (k > 0)
                    {
                      P(ind)(0) = zero;
                      P(ind)(1) = zero;
                      P(ind)(2) = Pow(x,i)*Pow(y,j)*Pow(z,k-1);
                      ind++;
                    }
                }
        }
              ...
              for (int j = 0; j <= r+1 ; j++)
                {
                  ...
                  P(ind)(0) = Pow(x,k)*Pow(y,i)*Pow(z,j);
                  P(ind)(1) = zero;
                  P(ind)(2) = zero;
                  ind++;
                  
                  P(ind)(0) = zero;
                  P(ind)(1) = Pow(x,j)*Pow(y,k)*Pow(z,i);
                  P(ind)(2) = zero;
                  ind++;
...
int main(int argc, char **argv) 
{
  ...
  int r = 4; int r_test = r;
  
  HexahedronHcurlFirst hex;
