				   ...
				   int n, const Storage2& stor)
{
  ...
  Matrix<complex<T>, General, Storage2> V, V2;
  ...
  // testing GetAbsoluteValue
  A = C;
  GetEigenvaluesEigenvectors(A, w, V);

  A = C;
  GetAbsoluteValue(A);
  
  Vector<complex<T> > cplx_w;
  GetEigenvaluesEigenvectors(A, cplx_w, V2);
  
  Vector<T> w_abs(w.GetM()), abs_w(w.GetM());
  Vector<Vector<complex<T> > > w_vec(w.GetM()), w2_vec(w.GetM());
  for (int i = 0; i < w.GetM(); i++)
    {
      w_abs(i) = abs(w(i));
      GetCol(V, i, w_vec(i));
      
      abs_w(i) = abs(cplx_w(i));
      GetCol(V2, i, w2_vec(i));
    }
  ...
  for (int i = 0; i < w.GetM(); i++)
    if ( (abs(w_abs(i) - abs_w(i)) > threshold)
	 || isnan (abs(w_abs(i) - abs_w(i)) ) )
      {
	cout << "GetAbsoluteValue incorrect" << endl;
	abort();
	
	if (!EqualVector(w_vec(i), w2_vec(i)))
	  {
	    cout << "GetAbsoluteValue incorrect" << endl;
	    abort();
	  }
  ...
  Sort(w_abs, w, w_vec);
  Sort(abs_w, cplx_w, w2_vec);

  // DISP(w); DISP(cplx_w);
  for (int i = 0; i < w.GetM(); i++)
    if ( (abs(w(i) - cplx_w(i)) > threshold)
	 || isnan (abs(w(i) - cplx_w(i)) ) )
      {
	cout << "GetExponential incorrect" << endl;
	abort();
...
template<class T, class Prop, class Storage, class Alloc, class Storage2>
void CheckSymmetricComplexEigenvalue(Matrix<complex<T>, Prop, Storage, Alloc>& A,
				     int n, const Storage2& stor)
{
  GenerateRandomMatrix(A, n, n);
  ...
  Vector<complex<T> > w2;
  GetEigenvaluesEigenvectors(A, w2, V2);
  
  Vector<T> w_abs(w.GetM()), w_abs2(w.GetM());
  Vector<Vector<complex<T> > > w_vec(w.GetM()), w2_vec(w.GetM());
  for (int i = 0; i < w.GetM(); i++)
    {
      w(i) = exp(w(i));
  ...
  Sort(w_abs, w, w_vec);
  Sort(w_abs2, w2, w2_vec);

  for (int i = 0; i < w.GetM(); i++)
    if ( (abs(w(i) - w2(i)) > threshold)
	 || isnan (abs(w(i) - w2(i)) ))
      {
	cout << "GetExponential incorrect" << endl;
	abort();
  ...
  for (int i = 0; i < w.GetM(); i++)
    if ( (abs(w_abs(i) - w_abs2(i)) > threshold)
	 || isnan (abs(w_abs(i) - w_abs2(i)) ))
      {
	cout << "GetAbsoluteValue incorrect" << endl;
	abort();
...
void CheckGeneralRealEigenvalue(Matrix<T, Prop, Storage, Alloc>& A, int n)
{
  ...
  // testing GetAbsoluteValue
  A = C;
  GetEigenvaluesEigenvectors(A, wr, wi, V);

  A = C;
  GetAbsoluteValue(A);
  
  Vector<T> abs_wr, abs_wi;
  GetEigenvaluesEigenvectors(A, abs_wr, abs_wi, V2);
  
  Vector<T> w_abs(wr.GetM());
  Vector<Vector<T> > w_vec(wr.GetM()), w2_vec(wr.GetM());
  for (int i = 0; i < wr.GetM(); i++)
    {
      w_abs(i) = abs(complex<T>(wr(i), wi(i)));
      GetCol(V, i, w_vec(i));
  ...
  for (int i = 0; i < wr.GetM(); i++)
    if ( (abs(w_abs(i) - abs_wr(i)) > threshold)
	 || isnan (abs(w_abs(i) - abs_wr(i)) ) || (abs(abs_wi(i)) > threshold) )
      {
	cout << "GetAbsoluteValue incorrect" << endl;
	abort();
  ...
  Vector<complex<T> > w(wr.GetM()), w2(wr.GetM());
  GetEigenvaluesEigenvectors(A, abs_wr, abs_wi, V2);
  
  for (int i = 0; i < wr.GetM(); i++)
    {
      w(i) = exp(complex<T>(wr(i), wi(i)));
      w_abs(i) = abs(w(i));
      GetCol(V, i, w_vec(i));

      w2(i) = complex<T>(abs_wr(i), abs_wi(i));
      abs_wr(i) = abs(w2(i));
      GetCol(V2, i, w2_vec(i));
    }

  Sort(w_abs, w, w_vec);
  Sort(abs_wr, w2, w2_vec);
  ...
  // testing GetSquareRoot (not correct for real matrices, since the result is a complex matrix)
  /* A = C;
  GetEigenvaluesEigenvectors(A, wr, wi, V);
				  ...
				  const Storage2& stor)
{
  ...
  Matrix<T, General, Storage2> V, V2;
  GetEigenvaluesEigenvectors(A, lambda, V);
  ...
  Vector<T> w_abs(w.GetM());
  Vector<Vector<T> > w_vec(w.GetM()), w2_vec(w.GetM());
  for (int i = 0; i < w.GetM(); i++)
    {
      w_abs(i) = exp(w(i));
      GetCol(V, i, w_vec(i));
  ...
  for (int i = 0; i < w.GetM(); i++)
    if ( (abs(w_abs(i) - w2(i)) > threshold)
	 || isnan (abs(w_abs(i) - w2(i)) ))
      {
	cout << "GetExponential incorrect" << endl;
	abort();
  ...
  for (int j = 0; j < n; j++)
    {
      ...
      T err = 0;
      for (int i = 0; i < n; i++)
        err += absSquare(y(i) - lambda(j)*x(i));
...
int main(int argc, char** argv)
{
    ...
    
  {
    ...
    int n = 14;
    CheckSymmetricComplexEigenvalue(A, n, col_major);
  }    
...

  {
    ...
    int n = 11;
    CheckSymmetricComplexEigenvalue(A, n, row_major);
  }    
