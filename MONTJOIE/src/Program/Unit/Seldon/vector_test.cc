...
void GenerateRandomVector(TinyVector<T, p>& x, int n)
{
  for (int i = 0; i < n; i++)
    GetRandNumber(x(i));
}
...
int main(int argc, char** argv)
{
  ...
  /*
  TinyVector<Real_wp, 5> x, y, z;
  GenerateRandomVector(x, 5);
  GenerateRandomVector(y, 5);
  GenerateRandomVector(z, 5);

  TinyVector<Complex_wp, 5> xc, yc, zc;
  GenerateRandomVector(xc, 5);
  GenerateRandomVector(yc, 5);
  GenerateRandomVector(zc, 5);

  Real_wp alpha; Complex_wp alphac;
  GetRandNumber(alpha); GetRandNumber(alphac);
  zc = x / yc;
  DISP(xc); DISP(alpha); DISP(zc);
  for (int i = 0; i < 5; i++)
    if (isnan(zc(i)) || isnan(x(i)*yc(i)) || abs(zc(i) - (x(i) / yc(i))) > threshold)
      {
	DISP(i); DISP(x(i));
	abort();
      }
