		 ...
		 Real_wp eps = threshold)
{
  ...
  for (int i = 0; i < x.GetM(); i++)
    if (isnan(abs(x(i) - y(i))) || (abs(x(i) - y(i)) > eps))
      return false;
...
void CheckHouseholder(Matrix<T, Prop, Storage>& A)
{
  int n = 20;
  Vector<T> U, Normale;
  ...
  // testing GetHouseholderNormale
  Normale = U;
  T alpha = GetHouseholderNormale(Normale);

  Vector<T> V;
  T scal = DotProdConj(Normale, U);

  V = U - (scal+scal)*Normale;
  for (int i = 0; i < V.GetM(); i++)
	...
	val_ref = alpha;
      ...
      if (isnan(abs(val_ref - V(i))) || (abs(val_ref - V(i)) > threshold))
	{
	  cout << "GetHouseholderNormale incorrect" << endl;
	  abort();
	}
    }

  Vector<T> V2 = U;
  ApplyHouseholderTransformation(Normale, 0, V2);

  if (!EqualVector(V, V2))
    {
      cout << "ApplyHouseholderTransformation incorrect " << endl;
      DISP(V); DISP(V2);
      abort();
    }
  ...
  int k = rand()%n + 1;
  Vector<T> X;
  GenerateRandomVector(X, n+k);

  Vector<T> Xsub(n);
  for (int i = 0; i < n; i++)
    Xsub(i) = X(k+i);

  scal = DotProdConj(Normale, Xsub);
  Xsub = Xsub - (scal+scal)*Normale;

  ApplyHouseholderTransformation(Normale, k, X);
  for (int i = 0; i < X.GetM(); i++)
    {
      T val_ref = X(i);
      if (i >= k)
	val_ref = Xsub(i-k);
      ...
      if (isnan(abs(val_ref - X(i))) || (abs(val_ref - X(i)) > threshold))
	{
	  cout << "ApplyHouseholderTransformation incorrect" << endl;
	  DISP(i); DISP(val_ref); DISP(X(i));
	  abort();
	}
    }


  // GetReorthogonalization not done
}
