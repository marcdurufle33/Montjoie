...
template<class T, class Prop, class Prop2, class Storage2>
void CheckRowMatrix(Matrix<T, Prop, ArrayRowSparse>& A0,
                    Matrix<T, Prop2, Storage2>& Ad)

{
  Matrix<T, Prop, ArrayRowSparse> A(A0);
  T zero, x, y, z, w;
  SetComplexZero(zero);
  GetRand(x); GetRand(y);
  GetRand(z); GetRand(w);
  ...
  Vector<T> row, row_d(A.GetN());
  A.GetDenseRow(irow, row);
  for (int j = 0; j < A.GetN(); j++)
    row_d(j) = Ad(irow, j);
  
  if (!EqualVector(row, row_d))
    {
      cout << "GetDenseRow incorrect" << endl;
      abort();
    }
  
  int icol = rand()%A.GetN();
  Vector<T> col, col_d(A.GetM());
  A.GetDenseCol(icol, col);
  for (int j = 0; j < A.GetM(); j++)
    col_d(j) = Ad(j, icol);
  
  if (!EqualVector(col, col_d))
    {
      cout << "GetDenseCol incorrect" << endl;
      abort();
    }

  row_d.Zero(); int nb_val = rand()%A.GetN();
  ...
  A.SetDenseRow(irow, row_d);
  if (!EqualMatrix(A, Ad))
    {
      cout << "SetDenseRow incorrect" << endl;
      abort();
    }

  col_d.Zero(); nb_val = rand()%A.GetM();
  for (int j = 0; j < nb_val; j++)
    {
      int j0 = rand()%A.GetM();      
      GetRand(col_d(j0));
  ...
  A.SetDenseCol(icol, col_d);
  
  if (!EqualMatrix(A, Ad))
    {
      cout << "SetDenseCol incorrect" << endl;
      abort();
    }
}
...
template<class T, class Prop, class Prop2, class Storage2>
void CheckRowMatrix(Matrix<T, Prop, ArrayRowSymSparse>& A0,
                    Matrix<T, Prop2, Storage2>& Ad0)

{
  Matrix<T, Prop, ArrayRowSymSparse> A(A0);
  Matrix<T, Prop2, Storage2> Ad(Ad0);
  T zero, x, y, z, w;
  SetComplexZero(zero);
  GetRand(x); GetRand(y);
  GetRand(z); GetRand(w);
