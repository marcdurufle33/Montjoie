...
void CheckProblem(const EllipticProblem<TypeEquation>& var_c, const T& z,
		  ...
		  bool check_trans = true, bool check_condensed = true)
{
  ...
  if (check_condensed)
	...
	for (int i = 0; i < x_sol.GetM(); i++)
      ...
      //DISP(var.GetNbScalarDof());
      //for (int i = 0; i < var.GetNbScalarDof()+24*79; i++)
      //  SetComplexZero(x_sol(i));
      
      //for (int i = var.GetNbScalarDof() + 24*79+1; i < var.GetNbDof(); i++)
      //  SetComplexZero(x_sol(i));
  ...
  //SetComplexOne(beta); SetComplexZero(gamma); SetComplexZero(alpha);
  
  int N = var.GetNbDof();
  DISP(N); DISP(var.mesh_num.GetNbDof());
  DISP(var.GetNbMainUnknownDof()); DISP(var.GetNbScalarDof());
	...
	if (check_trans)
	  {
	    ...
	    for (int j = 0; j < N; j++)
	      {
		T val = conjugate(alpha*Dh(i, j) + beta*Sh(i, j) + gamma*Kh(i, j));
		if ((var.FormulationDG() == ElementReference_Base::CONTINUOUS)
		    && var.IsDofDirichlet(j))
		  SetComplexZero(val);
		       ...
		       const string& name_element, const string& input_file)
{
  ...
  if (!EqualVector(X, Y, 10*threshold))
    {
...
int main(int argc, char** argv)
{
...

  {
    ...
    string input_file("cylinder_test.ini");
    string type_element, type_equation;
    getElement_Equation(input_file, type_element, type_equation);
    
    CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, true, true, true);
  ...
  // checking StaticMaxwellEquationHdg3D with HEXAHEDRON_HCURL_LOBATTO
  {
    // case with distorted hexahedral
    EllipticProblem<StaticMaxwellEquationHdg_3D> var;
    string input_file("src/Program/Unit/Instationary/cube_hexa.ini");
    //CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL_HDG", "HEXAHEDRON_HCURL_LOBATTO", input_file, true, true, false);
    CheckProblem(var, Complex_wp(0), true, "STATIC_MAXWELL_HDG", "HEXAHEDRON_HCURL_LOBATTO", input_file, false, true, true);
  }

  // checking HarmonicMaxwellEquationHdg3D with HEXAHEDRON_HCURL_LOBATTO
  {
    // case with distorted hexahedral
    EllipticProblem<HarmonicMaxwellEquationHdg_3D> var;
    string input_file("src/Program/Unit/Instationary/cube_hexa.ini");
    //CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL_HDG", "HEXAHEDRON_HCURL_LOBATTO", input_file, true, true, false);
    CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL_HDG", "HEXAHEDRON_HCURL_LOBATTO", input_file, false, true, true);
  }
  
  return FinalizeMontjoie();
  
  // checking HarmonicMaxwellEquation with HEXAHEDRON_FIRST_FAMILY
  {
    // case with curved elements (hybrid mesh)
    EllipticProblem<HarmonicMaxwellEquation_3D> var;
    string input_file("src/Program/Unit/Instationary/cube_max.ini");
    CheckProblem(var, Complex_wp(0), false, "HARMONIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, false, true, true);
    CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, false, true, true);
    CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, true, true, true);
  }
  ...
  // checking HarmonicMaxwellEquation with HEXAHEDRON_HCURL_LOBATTO
  {
    // case with affine elements and PML
    EllipticProblem<HarmonicMaxwellEquation_3D> var;
    string input_file("src/Program/Unit/Instationary/cube.ini");
    CheckProblem(var, Complex_wp(0), false, "HARMONIC_MAXWELL", "HEXAHEDRON_HCURL_LOBATTO", input_file, false, true, true);
    CheckProblem(var, Complex_wp(0), true, "HARMONIC_MAXWELL", "HEXAHEDRON_HCURL_LOBATTO", input_file, false, true, true);
  }

  // checking StaticMaxwellEquation with HEXAHEDRON_FIRST_FAMILY
  {
    EllipticProblem<StaticMaxwellEquation_3D> var;
    string input_file("src/Program/Unit/Instationary/cube_max.ini");
    CheckProblem(var, Complex_wp(0), false, "STATIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, false, true, true);
    CheckProblem(var, Complex_wp(0), true, "STATIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, false, true, true);
    CheckProblem(var, Complex_wp(0), true, "STATIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file, true, true, true);
    CheckEigenProblem(var, false, "STATIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file);
    CheckEigenProblem(var, true, "STATIC_MAXWELL", "HEXAHEDRON_FIRST_FAMILY", input_file);
  }
  ...
  // checking StaticMaxwellEquation with HEXAHEDRON_HCURL_LOBATTO
  {
    // case with affine elements and PML
    EllipticProblem<StaticMaxwellEquation_3D> var;
    string input_file("src/Program/Unit/Instationary/cube.ini");
    CheckProblem(var, Complex_wp(0), true, "STATIC_MAXWELL", "HEXAHEDRON_HCURL_LOBATTO", input_file, false, true, true);
    CheckEigenProblem(var, true, "STATIC_MAXWELL", "HEXAHEDRON_HCURL_LOBATTO", input_file);
  }
