                            ...
                            bool complex_mat = false, bool check_erase = true)
{
...

  {
    ...
    Atest.Init(GlobalRowNumbersTest, OverlapRowTest, OverlapProcTest,
               ProcSharingTest, SharingRowTest, comm);
  ...
  if (rank_processor == 0)
    Bref.WriteText(string("add_ref" + suffix + ".dat"));
  
  B.WriteText(string("add" + suffix + ".dat"));
  ...
  bool test_complex = !IsComplexMatrix(C);
#else
  bool test_complex = IsComplexMatrix(C);
#endif
  
  if (test_complex && (IsSymmetricMatrix(C)))
    {
  mat_lu.Factorize(C, false, true);  
  mat_lu_ref.Factorize(Cref, false, true);
...
int main(int argc, char** argv)
{
...

  {
    ...
    CheckDistributedMatrix(A, comm, suffix);
  }
