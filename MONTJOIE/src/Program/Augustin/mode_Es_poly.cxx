...
void ModeEsPolynomialProblem::RemoveDirichletRowCol(const Matrix<Complexe, Prop, Storage>& A,
...
void ModeEsPolynomialProblem::ExtractDirichlet(const VectComplex_wp& Xdir, VectComplex_wp& X)
{
  X.Zero();
...
void ModeEsPolynomialProblem::ExpandDirichlet(const VectComplex_wp& X, VectComplex_wp& Xdir)
{
  for (int i = 0; i < DofKeptDir.GetM(); i++)
...
void ModeEsPolynomialProblem::ComputeInvDBetaMatrix(const Complex_wp& L)
{
  int N = nb_dof_Hs;
...
void ModeEsPolynomialProblem::ComputeA0()
{
  A0.Clear();
                      ...
                      if (vec_twisted(ref))
                        {
                          ...
                          col(num) = jDir;
                          // partie - eps_sigma Mh V2t
                          val(num) = 4 * eps_sigma * k2 * tau * tau * contrib_Eigenvalue * vec_Mh(ref).Value(i, j);
                          num++;
                        }
                    }
                      ...
                      if (jDir >= 0)
                        {
                          col(num) = jDir;
                          // partie eps_sigma tau^2 Dh V2
                          val(num) = 2 * eps_sigma * tau * tau * k2 * contrib_Eigenvalue * vec_Th(ref).Value(i, j);
                          num++;
                        }
                    }
                  ...
                  if (vec_twisted(ref))
                    {
                      ...
                      col(num) = nb_dof_Es + j_Glob;
                      // partie - mu_tilde Mh W2t
                      val(num) = 4 * k2 * tau * tau * mu_tilde * contrib_Eigenvalue * vec_Mh(ref).Value(i, j);
                      num++;
                    }
                  ...
                  for (int j = 0; j < nT_Hs; j++)
                    {
                      ...
                      col(num) = nb_dof_Es + j_Glob;
                      // partie mu_tilde tau^2 Dh W2
                      val(num) = 2 * k2 * mu_tilde * tau * tau * contrib_Eigenvalue * vec_Th(ref).Value(i, j);
                      num++;
                    }
...
void ModeEsPolynomialProblem::ComputeA1()
{
  A1.Clear();
...
void ModeEsPolynomialProblem::ComputeA2()
{
  A2.Clear();
                  ...
                  if (jDir >= 0)
                    {
                      ...
                      val(num) = - 2 * eps_sigma * k2 * contrib_Eigenvalue * vec_Mh(ref).Value(i, j);
                      num++;
              ...
              for (int j = 0; j < nM_Hs; j++)
                {
                  ...
                  col(num) = nb_dof_Es + j_Glob;
                  // partie - mu_tilde Mh W2t
                  val(num) = - 2 * mu_tilde * k2 * contrib_Eigenvalue * vec_Mh(ref).Value(i, j);
                  num++;
...
void ModeEsPolynomialProblem::ComputeA3()
{
  A3.Clear();
...
void ModeEsPolynomialProblem::ComputeA4()
{
  A4.Clear();
...
void ModeEsPolynomialProblem::ComputeMatrix()
{
  ComputeA0();
  ComputeA1();
  ComputeA2();
