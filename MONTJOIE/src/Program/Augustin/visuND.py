...
def plot2dinst(X, Y, V, cmin='auto', cmax='auto', display_nan = True, log_scale = False,
  ...
  if (aspect_fig == None):
    if (log_scale):
      hh = pylab.imshow(V2, vmin=cmin, vmax=cmax, interpolation='bilinear', origin='lower', extent=(xmin,xmax,ymin,ymax), norm=matplotlib.colors.LogNorm(vmin=cmin, vmax=cmax), cmap=color_map)
    else:
