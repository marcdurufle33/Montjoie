...
ModeEsNonLinear_Dk_DirichletEigenProblem
::ModeEsNonLinear_Dk_DirichletEigenProblem(int N, ModeEs_Solver& var0,
  ...
  vec_Sih(Sih), vec_Sir2h(Sir2h), vec_Dh(Dh), vec_Dr2h(Dr2h), vec_Pos(Pos), DofKeptDir(DofDir), IndexNonDirichlet(IndexDir)
{
  ...
  if (use_split)
    {
      Vector<Complexe> num(1), denom(1);
      num(0) = 1.0; denom(0) = 1.0;
      ...
      num.Reallocate(3);
      num(2) = 0.0; num(1) = 0.0; num(0) = var.omega*var.omega; // à voir si je ne mets pas qu'un omega sur le beta^2 (modifier les matrices dans ce cas)
      this->numer_pol_split.PushBack(num);
      this->denom_pol_split.PushBack(denom);
    }
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::RemoveDirichletRowCol(const Matrix<Complexe, Prop, Storage>& A,
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ExtractDirichlet(const VectComplex_wp& Xdir, VectComplex_wp& X)
{
  X.Zero();
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ExpandDirichlet(const VectComplex_wp& X, VectComplex_wp& Xdir)
{
  for (int i = 0; i < DofKeptDir.GetM(); i++)
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ExtractDirichletAuxVariable(const VectComplex_wp& EHdir, const int& ref, VectComplex_wp& E3, VectComplex_wp& H3, VectComplex_wp& Fref, VectComplex_wp& Gref)
{
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ExpandDirichletAuxVariable(const int& ref, const VectComplex_wp& E3, const VectComplex_wp& H3, const VectComplex_wp& Fref, const VectComplex_wp& Gref, VectComplex_wp& EHdir)
{
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeOperator(const Complex_wp& L)
{

}

void ModeEsNonLinear_Dk_DirichletEigenProblem::Mult(const Matrix<Complexe, Prop, Storage>& A, const  Matrix<Complexe, Symmetric, DiagonalRow>& D, Matrix<Complexe, Prop, Storage>& M)
{
  int N = nb_dof_H3;
  M.Reallocate(N,N);
  for (int i = 0 ; i < N ; i++)
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeInvDBetaMatrix(const Complex_wp& L)
{
}

void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeChgmtVariable(const Complex_wp& L)
{
}


void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeEsHs(const Complex_wp& L,
    ...
    if (index_used(ref))
      {
        ...
        MltAdd(DBeta/k2, vec_Mh(ref), F, one, Es);
        MltAdd(- tau * tau, vec_Mr2h(ref), F, one, Es);
        MltAdd(mu_tilde * tau / k2, vec_Rjh(ref), G, one, Es);
        MltAdd(- mu_tilde * tau * tau * tau / DBeta, vec_Rjr2h(ref), G, one, Es);
        MltAdd(- 4.0 * mu_tilde * tau * tau * tau / DBeta, vec_Mr2h(ref), G, one, Es);
        MltAdd(Iwp * beta * tau / k2, vec_Sjh(ref), F, one, Es);
        MltAdd(- Iwp * beta * tau * tau * tau / DBeta, vec_Sjr2h(ref), F, one, Es);
...
// calcul de Y = T(L) X
void ModeEsNonLinear_Dk_DirichletEigenProblem::MltOperator(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
...
void ModeEsNonLinear_Dk_DirichletEigenProblem
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeJacobian(const Complex_wp& L)
{}

// calcul de Y = T'(L) X
void ModeEsNonLinear_Dk_DirichletEigenProblem::MltJacobian(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeExplicitPreconditioning(DistributedMatrix<Complex_wp, General, ArrayRowSparse>& A)
{
  mat_lu.Factorize(A);
}

// factorisation de T(L)
void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputePreconditioning(const Complex_wp& L)
{
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputePreconditioning(const VectComplex_wp& L, const VectComplex_wp& coef)
{
  cout << "Pas encore bien écrit : ComputePreconditioning" << endl;
  abort();
}

void ModeEsNonLinear_Dk_DirichletEigenProblem::ComputeSplitPreconditioning(const Vector<int>& numL, const VectComplex_wp& coef)
{
...
void ModeEsNonLinear_Dk_DirichletEigenProblem::ApplyPreconditioning(const SeldonTranspose&, const Vector<Complex_wp>& X, Vector<Complex_wp>& Y)
{
...
::ComputeOperatorSplitExplicit(int num, DistributedMatrix<Complex_wp, General, ArrayRowSparse>& A)
{
                    ...
                    if (jDir >= 0)
                      {
                        ...
                        col(num) = inc + j_loc_E3;
                        // partie - eps_sigma Mh V2t
                        val(num) = eps_sigma / k2 * vec_Kh(ref).Value(i, j);
                    ...
                    if (jDir >= 0)
                      {
                        ...
                        val(num) = - eps_sigma / k2 * tau * tau * vec_Dh(ref).Value(i, j);
                ...
                for (int j = 0; j < nK; j++)
                  {
                    ...
                    val(num) = mu_tilde / k2 * vec_Kh(ref).Value(i, j);
                ...
                for (int j = 0; j < nD; j++)
                  {
                    ...
                    val(num) = - mu_tilde * tau * tau / k2 * vec_Dh(ref).Value(i, j);
                    ...
                    if (jDir >= 0)
                      {
                        ...
                        col(num) = inc + j_loc_E3;
                        // partie eps_sigma Kh V2
                        val(num) = eps_sigma * tau / k2 * vec_Sjh(ref).Value(i, j);
                        num++;
                      }
                  }
                A.AddInteractionRow(iDir, num, col, val);
                    ...
                    if (jDir >= 0)
                      {
                        ...
                        col(num) = inc + j_loc_E3;
                        // partie eps_sigma Kh V2
                        val(num) = - eps_sigma * tau / k2 * vec_Sih(ref).Value(i, j);
                        num++;
                      }
                  }
                A.AddInteractionRow(iDir, num, col, val);
                ...
                for (int j = 0; j < nC; j++)
                  {
                    ...
                    col(num) = inc + nb_dof_coucheE3(ref) + j_loc;
                    // partie - eps_sigma Mh V2t
                    val(num) = 1.0 / k2 * vec_Ch(ref).Value(i, j);
                    num++;
                  }
                A.AddInteractionRow(iDir, num, col, val);
              }
                ...
                for (int j = 0; j < nSj; j++)
                  {
                    ...
                    col(num) = inc + nb_dof_coucheE3(ref) + j_loc;
                    // partie eps_sigma tau^2 Dh V2
                    val(num) = mu_tilde * tau / k2 * vec_Sjh(ref).Value(i, j);
                    num++;
                  }
                A.AddInteractionRow(nb_dof_E3 + i, num, col, val);
                ...
                for (int j = 0; j < nSi; j++)
                  {
                    ...
                    col(num) = inc + nb_dof_coucheE3(ref) + j_loc;
                    // partie eps_sigma tau^2 Dh V2
                    val(num) = - mu_tilde * tau / k2 * vec_Sih(ref).Value(i, j);
                    num++;
                  }
                A.AddInteractionRow(nb_dof_E3 + i, num, col, val);
                    ...
                    if (jDir >= 0)
                      {
                        ...
                        col(num) = inc + j_loc_E3;
                        // partie - eps_sigma Mh V2t
                        val(num) = eps_sigma / k2 * vec_Mh(ref).Value(i, j);
                        num++;
                      }
                  }
                A.AddInteractionRow(iDir, num, col, val);
                ...
                for (int j = 0; j < nM; j++)
                  {
                    ...
                    col(num) = inc + nb_dof_coucheE3(ref) + j_loc;
                    // partie eps_sigma Kh V2
                    val(num) = mu_tilde / k2 * vec_Mh(ref).Value(i, j);
                    num++;
                  }
                A.AddInteractionRow(nb_dof_E3 + i, num, col, val);
              }
  ...
  else if (num%2 == 1) // terme après f_3 = 1 /D_L(beta)
    {
      A.Clear();
      A.Reallocate(nb_rows, nb_rows);

      int ref = this->split_reference((num-3)/2);
      // DISP(num); DISP(ref);
      Complex_wp eps_sigma = -Iwp*var.omega*var.ref_epsilon(ref) + var.ref_sigma(ref);
      Complex_wp mu_tilde = -Iwp*var.omega*var.ref_mu(ref);
      Complex_wp k2 = -eps_sigma*mu_tilde;
  ...
  else if (num%2 == 0) // terme après f_3 = i beta /D_L(beta)
    {
      A.Clear();
      A.Reallocate(nb_rows, nb_rows);
...
::MltOperatorSplit(int num, const SeldonTranspose& trans, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
        ...
        if (index_used(ref))
          {
            ...
            MltAdd(eps_sigma / k2, vec_Kh(ref), F, one, Y1);
            MltAdd(mu_tilde / k2, vec_Kh(ref), G, one, Y2);
            MltAdd(-eps_sigma*tau*tau / k2, vec_Dh(ref), F, one, Y1);
            MltAdd(-mu_tilde*tau*tau / k2, vec_Dh(ref), G, one, Y2);
            ...
            for (int i = 0; i < NumGlob(ref).GetM(); i++)
              {
                int iGlob = NumGlob(ref)(i);
                int iDir = IndexNonDirichlet(iGlob);
                if (iDir >= 0)
                  {
                    // DISP(Var1(iGlob));
                    int iLoc = IndexDofE3(ref)(iGlob);
                    Ydir(inc + iLoc) = - Xdir(inc + iLoc) - Var1(iGlob) - Xdir(iDir); // <- - F - 2 tau^2 r^2 F - E3
                  }

                // DISP(Var2(iGlob));
                Ydir(inc + nb_dof_coucheE3(ref) + i) = - Xdir(inc + nb_dof_coucheE3(ref) + i) - Var2(iGlob) - Xdir(nb_dof_E3 + NumGlob(ref)(i));
              }
            inc += nb_dof_coucheE3(ref) + nb_dof_couche(ref);
          }
        ...
        if (index_used(ref))
          {
            ...
            // Équation principale
            MltAdd(eps_sigma*tau/k2, vec_Sjh(ref), F, one, Y1);
            MltAdd(mu_tilde*tau/k2, vec_Sjh(ref), G, one, Y2);
            MltAdd(-eps_sigma*tau/k2, vec_Sih(ref), F, one, Y1);
            MltAdd(-mu_tilde*tau/k2, vec_Sih(ref), G, one, Y2);
            MltAdd(1.0/k2, vec_Ch(ref), G, one, Y1);
            MltAdd(-1.0/k2, vec_Ch(ref), F, one, Y2);
        ...
        if (index_used(ref))
          {
            ...
            // Équation principale
            MltAdd(eps_sigma / k2, vec_Mh(ref), F, one, Y1);
            MltAdd(mu_tilde / k2, vec_Mh(ref), G, one, Y2);

            // Variables auxiliaires
            for (int i = 0; i < NumGlobE3(ref).GetM(); i++)
              {
                Ydir(inc + i) = 1.0/k2 * Xdir(inc + i);
              }
            for (int i = 0; i < NumGlob(ref).GetM(); i++)
              {
                Ydir(inc + nb_dof_coucheE3(ref) + i) = 1.0/k2 * Xdir(inc + nb_dof_coucheE3(ref) + i);
              }
            inc += nb_dof_coucheE3(ref) + nb_dof_couche(ref);
          }
  ...
  else if (num%2 == 1) // terme après f_2 = 1/D_L(beta)
    {
      Ydir.Reallocate(nb_rows); Ydir.Zero();
      int ref = this->split_reference((num - 3)/2);
      // DISP(num);
      ...
      MltAdd(-k2 * tau * tau * tau, vec_Rjr2h(ref), F, one, Y1);
      ...
      MltAdd(-k2 * tau * tau * tau, vec_Rir2h(ref), F, one, Y1);
      ...
      MltAdd(-4.0 * mu_tilde * tau * tau, vec_Rih(ref), G, one, Y1);
