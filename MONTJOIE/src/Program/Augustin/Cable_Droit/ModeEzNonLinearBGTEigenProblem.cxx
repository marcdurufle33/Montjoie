...
// calcul de Y = T(L) X
void ModeEzNonLinearBGTEigenProblem::MltOperator(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& X, Vector<Complex_wp>& Y)
{
  ...
  if (var.only_Ez)
  ...
  else
    {
        ...
        if (vec_Kh(ref).GetM() > 0)
          {
            ...
            Complex_wp Delta = eps_sigma * mu_tilde + beta*beta;


            MltAdd(eps_sigma, vec_Mh(ref), Ez, one, Y1);
            MltAdd(mu_tilde, vec_Mh(ref), Hz, one, Y2);
            MltAdd(eps_sigma/Delta, vec_Kh(ref), Ez, one, Y1);
            MltAdd(mu_tilde/Delta, vec_Kh(ref), Hz, one, Y2);
      ...
      Complex_wp Delta = eps_tilde * mu_tilde + beta*beta;
      Complex_wp SBeta = Sqrt(beta*beta - var.rhoInfini * var.omega*var.omega);
      Complex_wp XBeta = (3.0/4.0 + 3*var.ray*SBeta + 2*var.ray*var.ray*(beta*beta - var.rhoInfini * var.omega*var.omega)) / (2.0*var.ray*(1.0 + var.ray*SBeta)*Delta);
      Complex_wp YBeta = var.ray / (2.0*(1.0 + var.ray*SBeta)*Delta);

      MltAdd(eps_tilde*XBeta, matMh_bord, Ez, one, Y1);
      MltAdd(mu_tilde*XBeta, matMh_bord, Hz, one, Y2);
      MltAdd(eps_tilde*YBeta, Kh_bord, Ez, one, Y1);
      MltAdd(mu_tilde*YBeta, Kh_bord, Hz, one, Y2);
      MltAdd(Iwp * beta / Delta, Rh_bord, Hz, one, Y1);
      MltAdd(- Iwp * beta / Delta, Rh_bord, Ez, one, Y2);
