...
#include "ModeEzNonLinearFullDirichletEigenProblem.hxx"
                      ...
                      public VarFiniteElement<Dimension2>
{
  ...
  friend class ModeEzNonLinearFullDirichletEigenProblem;
...
protected:
  ...
  int type_CLA;
  enum {DIRICHLET, FULL_DIRICHLET, NEUMANN, BGT1, BGT2};
  bool linear;
...
public:
  ...
  { omega = 2.0*pi_wp; compute_laplace_eigen = false;
    ...
    type_coord_other_fields = NONE; type_CLA = DIRICHLET;
  ...
  void ResolutionNonLinearFullDirichlet(Vector<Complex_wp>& betas,
    ...
    if (!linear)
      {
        ...
        else if (type_CLA == FULL_DIRICHLET)
          {
            ResolutionNonLinearFullDirichlet(betas, Ez, Hz, vec_Mh, vec_Kh, vec_Ch);
          }
        ...
        if (type_CLA == DIRICHLET)
        ...
        else if (type_CLA == FULL_DIRICHLET)
          {
            if (only_Ez)
              ...
              {
                cout << "Full Dirichlet Linear not implement !" << endl;
                abort();
              }
          }
        else
        ...
        for (int i = 0; i < betas.GetM(); i++)
          {
            ...
            if (type_CLA == DIRICHLET)
              ...
              {
                VectComplex_wp X = x; X.Resize(2*nb_dof_Hz);
                VectComplex_wp Y = x; Y.Resize(2*nb_dof_Hz);
...
#include "ModeEzNonLinearFullDirichletEigenProblem.cxx"
