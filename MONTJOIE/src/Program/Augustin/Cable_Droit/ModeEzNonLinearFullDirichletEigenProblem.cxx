...
ModeEzNonLinearFullDirichletEigenProblem
::ModeEzNonLinearFullDirichletEigenProblem(int N, ModeEz_Solver& var0,
  ...
  :
  var(var0), vec_Mh(Mh), vec_Kh(Kh), vec_Ch(Ch), DofKeptNonDir(DofDir), IndexNonDirichlet(IndexDir)
{
  // pour Ez, condition de Dirichlet
  nb_dof_Ez = DofKeptNonDir.GetM();

  // pour Hz, condition de Dirichlet
  nb_dof_Hz = DofKeptNonDir.GetM();
  ...
  if (use_split)
    {
      ...
      Mh_sum.Reallocate(nb_node_mesh, nb_node_mesh);
      if (!var.only_Ez)
        Mh_sumH.Reallocate(nb_node_mesh, nb_node_mesh);
...
void ModeEzNonLinearFullDirichletEigenProblem::RemoveDirichletRowCol(const Matrix<Complexe, Prop, Storage>& A,
...
void ModeEzNonLinearFullDirichletEigenProblem::ExtractDirichlet(const VectComplex_wp& Xdir, VectComplex_wp& X)
{
  X.Zero();
  for (int i = 0; i < DofKeptNonDir.GetM(); i++)
    X(DofKeptNonDir(i)) = Xdir(i);

  if (!var.only_Ez)
    for (int i = 0; i < DofKeptNonDir.GetM(); i++)
      X(nb_node_mesh + DofKeptNonDir(i)) = Xdir(nb_dof_Ez + i);
}

void ModeEzNonLinearFullDirichletEigenProblem::ExpandDirichlet(const VectComplex_wp& X, VectComplex_wp& Xdir)
{
  for (int i = 0; i < DofKeptNonDir.GetM(); i++)
    Xdir(i) = X(DofKeptNonDir(i));

  if (!var.only_Ez)
    for (int i = 0; i < DofKeptNonDir.GetM(); i++)
      Xdir(nb_dof_Ez + i) = X(nb_node_mesh + DofKeptNonDir(i));
}

void ModeEzNonLinearFullDirichletEigenProblem::ComputeOperator(const Complex_wp& L)
{
}


// calcul de Y = T(L) X
void ModeEzNonLinearFullDirichletEigenProblem::MltOperator(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
  cout << "MltOperator not implemented" << endl;
  abort();
...
void ModeEzNonLinearFullDirichletEigenProblem::ComputeJacobian(const Complex_wp& L)
{}

// calcul de Y = T'(L) X
void ModeEzNonLinearFullDirichletEigenProblem::MltJacobian(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
  cout << "MltJacobian not implemented" << endl;
  abort();
  // Vector<Complex_wp> X(nb_dof_all), Y(nb_dof_all);
  // ExtractDirichlet(Xdir, X);
  // Y.Zero();
...
{
  cout << "ComputeJacobianExplicit not implemented" << endl;
  abort();
  // A.Reallocate(nb_rows, nb_rows);
  //
  // Complex_wp beta = L*var.omega;
...
void ModeEzNonLinearFullDirichletEigenProblem::ComputeExplicitPreconditioning(DistributedMatrix<Complex_wp, General, ArrayRowSparse>& A)
{
  mat_lu.Factorize(A);
}

// factorisation de T(L)
void ModeEzNonLinearFullDirichletEigenProblem::ComputePreconditioning(const Complex_wp& L)
{
...
void ModeEzNonLinearFullDirichletEigenProblem::ComputePreconditioning(const VectComplex_wp& L, const VectComplex_wp& coef)
{
...
void ModeEzNonLinearFullDirichletEigenProblem::ComputeSplitPreconditioning(const Vector<int>& numL, const VectComplex_wp& coef)
{
...
void ModeEzNonLinearFullDirichletEigenProblem::ApplyPreconditioning(const SeldonTranspose&, const Vector<Complex_wp>& X, Vector<Complex_wp>& Y)
{
...
::ComputeOperatorSplitExplicit(int num, DistributedMatrix<Complex_wp, General, ArrayRowSparse>& A)
{
  if (var.only_Ez)
    {
      cout << "OnlyEz not implemented" << endl;
      abort();
    }
  else
    {
      ...
      if (num == 0)
        {
          A.Clear();
          A.Reallocate(nb_rows, nb_rows);
          for (int i = 0; i < nb_node_mesh; i++)
            {
              // partie (-i omega epsilon + sigma) M_h Ez
              int i0 = IndexNonDirichlet(i);
              int nb_val = 0;
              for (int j = 0; j < Mh_sum.GetRowSize(i); j++)
                if (IndexNonDirichlet(Mh_sum.Index(i, j)) >= 0)
                  nb_val++;
              ...
              if (i0 >= 0)
                {
                  ...
                  for (int j = 0; j < Mh_sum.GetRowSize(i); j++)
                    if (IndexNonDirichlet(Mh_sum.Index(i, j)) >= 0)
                      {
                        A.Index(i0, nb_val) = IndexNonDirichlet(Mh_sum.Index(i, j));
              ...
              nb_val = 0;
              for (int j = 0; j < Mh_sumH.GetRowSize(i); j++)
                if (IndexNonDirichlet(Mh_sumH.Index(i, j)) >= 0)
                  nb_val++;

              if (i0 >= 0)
                {
                  A.ReallocateRow(nb_dof_Ez + i0, nb_val);
                  nb_val = 0;
                  for (int j = 0; j < Mh_sumH.GetRowSize(i); j++)
                    if (IndexNonDirichlet(Mh_sumH.Index(i, j)) >= 0)
                      {
                        A.Index(nb_dof_Ez + i0, nb_val) = nb_dof_Ez + IndexNonDirichlet(Mh_sumH.Index(i, j));
                        A.Value(nb_dof_Ez + i0, nb_val) = Mh_sumH.Value(i, j);
                        nb_val++;
                      }
                }
            }
          ...
          for (int i = 0; i < nb_node_mesh; i++)
            {
              ...
              for (int j = 0; j < vec_Kh(ref).GetRowSize(i); j++)
                if (IndexNonDirichlet(vec_Kh(ref).Index(i, j)) >= 0)
                  nb_val++;
              ...
              if (i0 >= 0)
                {
                  ...
                  for (int j = 0; j < vec_Kh(ref).GetRowSize(i); j++)
                    if (IndexNonDirichlet(vec_Kh(ref).Index(i, j)) >= 0)
                      {
                        // partie (-i omega epsilon + sigma)/Delta K_h Ez
                        A.Index(i0, nb_val) = IndexNonDirichlet(vec_Kh(ref).Index(i, j));
                        A.Value(i0, nb_val) = eps_sigma*vec_Kh(ref).Value(i, j);

                          // partie (-i omega mu)/Delta K_h Ez
                        A.Index(nb_dof_Ez + i0, nb_val) = nb_dof_Ez + IndexNonDirichlet(vec_Kh(ref).Index(i, j));
                        A.Value(nb_dof_Ez + i0, nb_val) = coef_mu*vec_Kh(ref).Value(i, j);
                        nb_val++;
                      }
                }
            }
          ...
          for (int i = 0; i < nb_node_mesh; i++)
            {
              ...
              for (int j = 0; j < vec_Ch(ref).GetRowSize(i); j++)
                if (IndexNonDirichlet(vec_Ch(ref).Index(i, j)) >= 0)
                  nb_val++;
                  ...
                  for (int j = 0; j < nb_val; j++)
                    {
                      if (IndexNonDirichlet(vec_Ch(ref).Index(i, j)) >= 0)
                        {
                          // partie i beta/Delta C_h Hz
                          A.Index(i0, nb_val) = nb_dof_Ez + IndexNonDirichlet(vec_Ch(ref).Index(i, j));
                          A.Value(i0, nb_val) = vec_Ch(ref).Value(i, j);

                          // partie -i beta/Delta C_h E_z
                          A.Index(nb_dof_Ez + i0, nb_val) = IndexNonDirichlet(vec_Ch(ref).Index(i, j));
                          A.Value(nb_dof_Ez + i0, nb_val) = -vec_Ch(ref).Value(i, j);
