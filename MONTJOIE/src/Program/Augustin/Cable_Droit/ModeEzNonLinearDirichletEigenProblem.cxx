...
ModeEzNonLinearDirichletEigenProblem
::ModeEzNonLinearDirichletEigenProblem(int N, ModeEz_Solver& var0,
...
void ModeEzNonLinearDirichletEigenProblem::RemoveDirichletRowCol(const Matrix<Complexe, Prop, Storage>& A,
...
void ModeEzNonLinearDirichletEigenProblem::ExtractDirichlet(const VectComplex_wp& Xdir, VectComplex_wp& X)
{
  X.Zero();
...
void ModeEzNonLinearDirichletEigenProblem::ExpandDirichlet(const VectComplex_wp& X, VectComplex_wp& Xdir)
{
  for (int i = 0; i < DofKeptNonDir.GetM(); i++)
...
void ModeEzNonLinearDirichletEigenProblem::ComputeOperator(const Complex_wp& L)
{
}


// calcul de Y = T(L) X
void ModeEzNonLinearDirichletEigenProblem::MltOperator(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
  count++;
  if (count%1000 ==0)
    cout << "MltOperator " << count << endl;
...
void ModeEzNonLinearDirichletEigenProblem
...
void ModeEzNonLinearDirichletEigenProblem::ComputeJacobian(const Complex_wp& L)
{}

// calcul de Y = T'(L) X
void ModeEzNonLinearDirichletEigenProblem::MltJacobian(const Complex_wp& L, const SeldonTranspose&, const Vector<Complex_wp>& Xdir, Vector<Complex_wp>& Ydir)
{
...
{
  // cout << "ComputeJacobianExplicit" << endl;
  A.Reallocate(nb_rows, nb_rows);
...
void ModeEzNonLinearDirichletEigenProblem::ComputeExplicitPreconditioning(DistributedMatrix<Complex_wp, General, ArrayRowSparse>& A)
{
...
void ModeEzNonLinearDirichletEigenProblem::ComputePreconditioning(const Complex_wp& L)
{
  cout << "ComputePreconditioning 1" << endl;
...
void ModeEzNonLinearDirichletEigenProblem::ComputePreconditioning(const VectComplex_wp& L, const VectComplex_wp& coef)
{
  cout << "ComputePreconditioning 2" << endl;
...
void ModeEzNonLinearDirichletEigenProblem::ComputeSplitPreconditioning(const Vector<int>& numL, const VectComplex_wp& coef)
{
...
void ModeEzNonLinearDirichletEigenProblem::ApplyPreconditioning(const SeldonTranspose&, const Vector<Complex_wp>& X, Vector<Complex_wp>& Y)
{
          ...
          for (int i = 0; i < nb_dof_Hz; i++)
            {
              ...
              for (int j = 0; j < vec_Ch(ref).GetRowSize(i); j++)
                if (IndexNonDirichlet(vec_Ch(ref).Index(i, j)) >= 0)
                  {
                    A.Index(nb_dof_Ez + i, nb_val) = IndexNonDirichlet(vec_Ch(ref).Index(i, j));
