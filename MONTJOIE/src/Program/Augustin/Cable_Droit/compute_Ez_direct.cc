...
public:
  ...
  void SetInputData(const string& keyword, const VectString& parameters)
  {
	...
	var_grid(i).SetInputData(keyword, parameters);
    ...
    else if (keyword == "Condition")
      {
        ...
        if ((parameters(0) == "Neumann") || (parameters(0) == "Dirichlet"))
          {
            cout << " " << parameters(1);
          }
        else
            ...
            if (realpart(rhoInfini) == 0.0)
              {
                ...
                rhoInfini = this->ref_epsilon(refElem)*this->ref_mu(refElem)
                            + Iwp * this->ref_sigma(refElem) * this->ref_mu(refElem) / this->omega;
              }
            int nb_dof = Fb.GetNbDofBoundary(num_loc);
            for (int j = 0; j < nb_dof; j++)
              {
  ...
  void RunAll(const string& input_file)
  {
    ...
    //   {
    //     Complex_wp rho = 0.0;
    //     Complex_wp eps = this->ref_epsilon(ref);
    //     Complex_wp mu = this->ref_mu(ref);
    //     Complex_wp sigma = this->ref_sigma(ref);
    //     rho = eps*mu + Iwp * sigma * mu / this->omega;
    //     cout << "Ref " << ref << " : rho = " << rho << endl;
    //   }
