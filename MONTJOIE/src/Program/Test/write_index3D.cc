...
void ComputeIndex(const Real_wp& x, const Real_wp& y, const Real_wp& z, int n, Complexe& coef)
{
  /* if (n == 0)
  ...
  if (n == 0)
    coef = 0.8 + x*y*z + 0.3*x*y*y - 0.5*x*x*z + 0.4*y*z*z;
  //coef = 1.5 + 0.2*cos(pi_wp*x/4)*sin(pi_wp*y/2);
  ...
  else
    {
      ...
      if (n == 2)
        ...
        {
          //coef = 0.6 + 0.3*sin(pi_wp*x/2)*cos(pi_wp*y/4);
          if (n == 4)
            coef = 0.3*pi_wp/2*cos(pi_wp*x/2)*cos(pi_wp*y/4);
          else
            coef = -0.3*pi_wp/4*sin(pi_wp*x/2)*sin(pi_wp*y/4);
        }
    }
}
...
int main(int argc, char** argv)
{
  ...
  if (argc < 3)
    {
      cout << "Usage : write_index nodal_points.dat rho.don mu.don sigma.don ..." << endl;
      abort();
    }
  ...
  Vector<R3> points(nb_nodes);
  for (int i = 0; i < nb_nodes; i++)
    file_in >> points(i)(0) >> points(i)(1) >> points(i)(2);
  ...
  for (int j = 0; j < name_dest.GetM(); j++)
    {
      ...
      for (int i = 0; i < nb_nodes; i++)
	ComputeIndex(points(i)(0), points(i)(1), points(i)(2), j, nu_node(j)(i));
    }
