#ifndef MONTJOIE_FILE_PYRAMID_HDIV_HP_FIRST_FAMILY_CXX

namespace Montjoie
{
  
  //! default constructor
  PyramidHdivHpFirstFamily::PyramidHdivHpFirstFamily() : PyramidReference<3>()
  {
  ...
  //! how to number mesh
  void PyramidHdivHpFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
  ...
  size_t PyramidHdivHpFirstFamily::GetMemorySize() const
  {
  ...
  void PyramidHdivHpFirstFamily::ConstructFiniteElement(int r, int rgeom, int rquad, int type_quad,
							...
							int type_surf_tri, int type_surf_quad, int gauss_z)
  {
    ...
    //this->mass_matrix.Write("MhJr_First"+to_str(r)+".dat");
    //this->const_div_matrix.Write("KhJr_First"+to_str(r)+".dat");
  ...
  */
  void PyramidHdivHpFirstFamily::ComputeValuesPhiRef(const R3& point_loc, VectR3& res) const
  {
  ...
  */
  void PyramidHdivHpFirstFamily::ComputeDivPhiRef(const R3& point_loc, VectReal_wp& res) const
  {
    res.Reallocate(nb_dof_loc);
	...
	for (int k = 0; k <= order -1 - max(i, j); k++)
	  {
	    ...
	    if (j < order-1)
	      res(num++) = val*div_v6 + DotProd(grad, v6);
	    
	    if (i < order-1)
	      res(num++) = val*div_v7 + DotProd(grad, v7);	    
	  }
  }
  ...
  //! displays details of class PyramidHdivHpFirstFamily
...
#define MONTJOIE_FILE_PYRAMID_HDIV_HP_FIRST_FAMILY_CXX
#endif
