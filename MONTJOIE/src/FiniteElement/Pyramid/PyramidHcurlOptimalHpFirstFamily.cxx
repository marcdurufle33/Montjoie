#ifndef MONTJOIE_FILE_PYRAMID_HCURL_OPTIMAL_HP_FIRST_FAMILY_CXX

namespace Montjoie
{

  //! default constructor
  PyramidHcurlOptimalHpFirstFamily::PyramidHcurlOptimalHpFirstFamily() : PyramidReference<2>()
  {
  ...
  //! how to number mesh
  void PyramidHcurlOptimalHpFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
  ...
  size_t PyramidHcurlOptimalHpFirstFamily::GetMemorySize() const
  {
  ...
  void PyramidHcurlOptimalHpFirstFamily
			   ...
			   int type_surf_tri, int type_surf_quad, int gauss_z)
  {
    ...
    //Mh.Write("MhPyrCurl"+to_str(this->order)+".dat");
    //Kh.Write("KhPyrCurl"+to_str(this->order)+".dat");
  }
  
  
  //! construction of basis functions
  void PyramidHcurlOptimalHpFirstFamily::ConstructFunctions()
  {
  ...
  */
  ...
  ::ComputeValuesPhiRef(const R3& point_loc, VectR3& phi) const
  {
    ...
    for (int i = 0; i <= order-2; i++)
      EvaluateJacobiPolynomial(EvenJacobiPolynom(i), order-2-i, c, Pz(i));
    ...
    for (int i = 0; i <= order-1; i++)
	  ...
	  node = offset + NumDofsY_quad(i, j);
          phi(node) = v4;
          phi(node) *= beta3*Jx(i)*Py(j)*powOneMinusZ(max(i,j))*invOmz;
        }
  ...
  */
  void PyramidHcurlOptimalHpFirstFamily::ComputeCurlPhiRef(const R3& point_loc, VectR3& res) const
  {
    ...
    for (int i = 0; i <= order-2; i++)
      EvaluateJacobiPolynomial(EvenJacobiPolynom(i), order-2-i, c, Pz(i), dPz(i));
    ...
    for (int i = 0; i <= order-1; i++)
	  ...
	  node = offset + NumDofsY_quad(i, j);
	  val = vz2*Jx(i)*Py(j)*powOneMinusZ(m);	  
	  grad_val.Init(Py(j)*powOneMinusZ(m)*(da_dx*dJx(i)*vz2 + Jx(i)*dvz2(0)),
			Jx(i)*powOneMinusZ(m)*vz2*dPy(j)*db_dy,
			vz2*powOneMinusZ(m)*(dJx(i)*da_dz*Py(j) + Jx(i)*dPy(j)*db_dz)
			+ Jx(i)*Py(j)*(vz2*dpowOneMinusZ(m) + dvz2(2)*powOneMinusZ(m)));
    ...
    for (int i = 0; i <= order-2; i++)
	  ...
	  dvz.Init(powOneMinusZ(m)*invOmz*dPx(i)*Py(j)*da_dx,
	      ...
	      grad_val(2) += vz2*(-0.5*lambda5 + beta4);
              //DISP(i); DISP(j); DISP(k); DISP(vz); DISP(dvz);
              //DISP(vz2); DISP(dvz2); DISP(val); DISP(grad_val); DISP(v1); DISP(curl_v1);
  ...
  //! displays details of class PyramidHcurlOptimalHpFirstFamily
...
#define MONTJOIE_FILE_PYRAMID_HCURL_OPTIMAL_HP_FIRST_FAMILY_CXX
#endif
