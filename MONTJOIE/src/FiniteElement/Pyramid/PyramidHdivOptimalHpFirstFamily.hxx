#ifndef MONTJOIE_FILE_PYRAMID_HDIV_OPTIMAL_HP_FIRST_FAMILY_HXX

namespace Montjoie
{
  
  //! finite element class for optimal Nedelec's first family on pyramids
  /*!
    Hierarchical basis functions are constructed by using Jacobi polynomials
    and generate the following finite element space
    C_{r-1}^3 + {x^i y^j (1-z)^r [x, y, -1], i, j <= r}
    + { x^(n+1) y^m (1-z)^n [1, 0, 0], x^m y^(n+1) (1-z)^n [0, 1, 0], 0 <= m <= n <= r-1 }
    + { x^m y^(n+1) (1-z)^n [x, 0, -1], x^(n+1) y^m (1-z)^n [0, y, -1], 0 <= m <= n <= r-1 }
   ...
   */
  class PyramidHdivOptimalHpFirstFamily : public PyramidReference<3>
  {
  protected :
...
#define MONTJOIE_FILE_PYRAMID_HDIV_OPTIMAL_HP_FIRST_FAMILY_HXX
#endif
