#ifndef MONTJOIE_FILE_PYRAMID_DG_ORTHO_HXX

namespace Montjoie
{
  
  //! class for orthogonal finite element on pyramids and with DG formulation
  ...
  /*!
    ...
    Jacobi polynomials. These basis functions generate the space
    P_r(x, y, z) + \sum_{k=0}^{r-1} P_k(x, y) (x y/(1-z))^(r-k) 
   */
  class PyramidDgOrtho : public PyramidReference<1>
  {
  protected :
    ...
    Vector<Real_wp> beta, gamma;
    Array2D<Matrix <Real_wp> > CmO1, CpO1, CmO2, CpO2, CmO1mO2, CpO1mO2, CmO1pO2, CpO1pO2;
  ...
  public :
    ...
    void MltOptimalMass(const VectReal_wp& coef, const VectReal_wp& Uh, VectReal_wp& Vh) const;
  ...
  template<class T>
  TinyVector<Vector<T>, 20> PyramidDgOrthoVariables<T>::Uquad;

  template<class T>
  TinyVector<Vector<T>, 20> PyramidDgOrthoVariables<T>::Ux;
  
  template<class T>
  TinyVector<Vector<T>, 20> PyramidDgOrthoVariables<T>::Uy;
  
  template<class T>
  TinyVector<Vector<T>, 20> PyramidDgOrthoVariables<T>::Uz;
  
  template<class T>
  TinyVector<Vector<T>, 20> PyramidDgOrthoVariables<T>::Uy2;
  
} // namespace Montjoie

#define MONTJOIE_FILE_PYRAMID_DG_ORTHO_HXX
#endif
