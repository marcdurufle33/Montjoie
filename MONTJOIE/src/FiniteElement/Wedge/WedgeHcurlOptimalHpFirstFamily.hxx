#ifndef MONTJOIE_FILE_WEDGE_HCURL_OPTIMAL_HP_FIRST_FAMILY_HXX

namespace Montjoie
{
  
  //! finite element class for optimal Nedelec's first family on triangular prisms
  /*!
    Hierarchical basis functions are constructed with Jacobi polynomials
    and generate the following finite element space :
    R_r(x, y) P_{r+1}(z)  x  P_{r+1}(x, y) P_{r-1}(z)
    with R_r Nedelec's first family on triangular element
   */
  class WedgeHcurlOptimalHpFirstFamily : public WedgeReference<2>
  {
  protected :
...
#define MONTJOIE_FILE_WEDGE_HCURL_OPTIMAL_HP_FIRST_FAMILY_HXX
#endif
