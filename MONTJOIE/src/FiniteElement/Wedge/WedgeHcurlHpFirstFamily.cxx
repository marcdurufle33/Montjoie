#ifndef MONTJOIE_FILE_WEDGE_HCURL_HP_FIRST_FAMILY_CXX

namespace Montjoie
{
  
  //! default constructor
  WedgeHcurlHpFirstFamily::WedgeHcurlHpFirstFamily() : WedgeReference<2>()
  {
  ...
  //! how to number mesh
  void WedgeHcurlHpFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
  ...
  size_t WedgeHcurlHpFirstFamily::GetMemorySize() const
  {
  ...
  void WedgeHcurlHpFirstFamily::ConstructFiniteElement(int r, int rgeom, int rquad, int type_quad,
  ...
  void WedgeHcurlHpFirstFamily::ConstructFunctions()
  {
    ...
    TriangleHcurlFirstFamily element_tri_hcurl;
  ...
  */
  void WedgeHcurlHpFirstFamily::ComputeValuesPhiRef(const R3& point_loc, VectR3& phi) const
  {
  ...
  */
  void WedgeHcurlHpFirstFamily::ComputeCurlPhiRef(const R3& point_loc, VectR3& res) const
  {
    ...
    for (int i = 0; i <= order-1; i++)
	  ...
	  node = offset + NumDofsX_quad(i, j);
          ...
          grad_lambda.Init((1.0-2.0*x-y)*Sxizj + 2.0*(1.0-x-y)*x*dJx(j)*Pz(i),
                           -x*Sxizj, 0);
    ...
    for (int i = 0; i <= order-1; i++)
	  ...
	  node = offset + NumDofsX_quad(i, j);
          ...
          Syizj = Jy(j)*Pz(i);
          grad_lambda.Init(y*Syizj, x*Syizj + 2.0*x*y*dJy(j)*Pz(i), 0);
    ...
    for (int i = 0; i <= order-1; i++)
	  ...
	  node = offset + NumDofsX_quad(i, j);
          ...
          Syizj = Jy(j)*Pz(i);
          grad_lambda.Init(-y*Syizj, (1.0-x-2.0*y)*Syizj + 2.0*(1.0-x-y)*y*dJy(j)*Pz(i), 0);
          node = offset + NumDofsY_quad(j, i);
          res(node)(0) = grad_lambda(1);
          res(node)(1) = -grad_lambda(0);
          res(node)(2) = 0;
    ...
    for (int k = 0; k <= order-1; k++)
      for (int i = 0; i <= order-2; i++)
        for (int j = 0; j <= order-2; j++)
          if (i+j <= order-2)
            {
              ...
              if (k < order - 1)
                {
		  node = NumDofsX(i, j, k);
                  res(node)(0) = (-1+x)*x*Px(i)*Py(j)*(-Pz(k)+2*z*Pz(k)
                                                       +2*pow(z,2)*dPz(k)-2*z*dPz(k));
		  ...
		  node = NumDofsY(i, j, k);
                  res(node)(0) = (1-x-y)*x*Px(i)*Py(j)
                    *(-Pz(k)+2*z*Pz(k)+2*pow(z,2)*dPz(k)-2*z*dPz(k));
                  
                  res(node)(1) = (1-x-y)*y*Px(i)*Py(j)
                    *(-Pz(k)+2*z*Pz(k)+2*pow(z,2)*dPz(k)-2*z*dPz(k));
                  res(node)(2) = -(1-z)*z*Pz(k)*(2*pow(y,2)*dPy(j)*Px(i)
                                                 +3*x*Px(i)*Py(j)+3*y*Px(i)*Py(j)-2*Px(i)*Py(j)
						 -2*x*dPx(i)*Py(j)+2*y*x*dPy(j)*Px(i)
                                                 +2*pow(x,2)*dPx(i)*Py(j)+2*x*y*dPx(i)*Py(j)
                                                 -2*y*dPy(j)*Px(i));
                }
...
#define MONTJOIE_FILE_WEDGE_HCURL_HP_FIRST_FAMILY_CXX
#endif
