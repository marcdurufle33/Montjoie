#ifndef MONTJOIE_FILE_WEDGE_HDIV_FIRST_FAMILY_CXX

namespace Montjoie
{
  
  //! default constructor
  WedgeHdivFirstFamily::WedgeHdivFirstFamily() : WedgeReference<3>()
  {
  ...
  size_t WedgeHdivFirstFamily::GetMemorySize() const
  {
  ...
  //! how to number mesh
  void WedgeHdivFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
    ...
    OppositeSigneDof.Reallocate(8, order*order);
    OppositeSigneDof.Fill(false);
    for (int i = 4; i < 8; i++)
  ...
  void WedgeHdivFirstFamily::ConstructFiniteElement(int r, int rgeom, int rquad, int type_quad,
  ...
  void WedgeHdivFirstFamily::ConstructFunctions()
  {
    Fb_tri.ConstructFiniteElement(order);
    if (order > 1)
    ...
    lob_gauss.ConstructQuadrature(order-1, Globatto<Real_wp>::QUADRATURE_GAUSS);
    lob_lobatto.ConstructQuadrature(order, Globatto<Real_wp>::QUADRATURE_LOBATTO);
    
    points_dof2d_quad.Reallocate(order*order);
    for (int i = 0; i < order; i++)
      for (int j = 0; j < order; j++)
	points_dof2d_quad(i*order + j).Init(lob_gauss.Points(i), lob_gauss.Points(j));
      
    NumDofsXY.Reallocate(Fb_tri.GetNbDof(), order);
    NumDofsZ.Reallocate(Ntri, order+1);
    NumDofsXY.Fill(-1);
    NumDofsZ.Fill(-1);
    ...
    int Nquad = order*order;
    for (int i = 0; i < order; i++)
      for (int j = 0; j < order; j++)
	{
	  NumDofsXY(i, j) = Ntri + i*order + j;
	  NumDofsXY(order+i, j) = Ntri + Nquad + i*order + j;
	  NumDofsXY(2*order+i, j) = Ntri + 2*Nquad + (order-1-i)*order + j;
	}
    
    // dofs of the last triangular face
    for (int i = 0; i < Ntri; i++)
      NumDofsZ(i, order) = Ntri + 3*Nquad + i;
    
    // dofs inside
    int nb = 2*Ntri + 3*Nquad;
    for (int i = 3*order; i < Fb_tri.GetNbDof(); i++) 
      for (int j = 0; j < order; j++)
	NumDofsXY(i, j) = nb++;
  ...
  */
  void WedgeHdivFirstFamily::ComputeValuesPhiRef(const R3& point_loc, VectR3& res) const
  {
  ...
  */
  void WedgeHdivFirstFamily::ComputeDivPhiRef(const R3& point_loc, VectReal_wp& res) const
  {
    res.Reallocate(nb_dof_loc);
    res.Fill(0);
  ...
  */
  void WedgeHdivFirstFamily::ComputeValuesPhiOrthoRef(const R3& pointloc, VectR3& phi) const
  {
    ...
    EvaluateJacobiPolynomial(LegendrePolynom, r+1, c, Pz);
  ...
  */  
  ...
  //! displays details of class WedgeHdivFirstFamily
  ostream& operator <<(ostream& out, const WedgeHdivFirstFamily& e)
  {
...
#define MONTJOIE_FILE_WEDGE_HDIV_FIRST_FAMILY_CXX
#endif
