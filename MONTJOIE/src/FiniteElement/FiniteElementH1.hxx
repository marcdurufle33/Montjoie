#ifndef MONTJOIE_FILE_FINITE_ELEMENT_H1_HXX
...
namespace Montjoie
{
  ...
  class FiniteElementH1
  {
  public :
    enum{nb_components_u = 1, nb_components_grad = Dimension::dim_N};
    ...
    inline static int GetNatureElement() { return 1; }
    
    // computes res(i) = \phi_i(point_loc) for all basis function \phi_i
    void ComputeValuesPhi(const R_N& point_loc, Vector<TinyVector<Real_wp,1> >& res,
                          const MatrixN_N& dfjm1, const MeshNumbering<Dimension>&, int nquad) const;
    
    // computes res(i) = \nabla \phi_i (point_loc)
    void ComputeValuesGradientPhi(const R_N& point_loc, Vector<R_N>& res, const MatrixN_N& dfjm1,
				  const MeshNumbering<Dimension>&, int nquad) const;
    ...
    void ComputeValuesPhiBoundary(Matrix<TinyVector<Real_wp, 1> >& ValuePhi_Boundary,
				  const SetMatrices<Dimension>& MatricesElem,
				  const MeshNumbering<Dimension>&, int nquad, int num_loc) const;
    
    // computes Gradient_Boundary(i,k) = d\phi_i / dx (\xi_k),
    // where \xi_k is a quadrature point on the edge num_loc
    void ComputeValuesGradientPhiBoundary(Matrix<R_N>& GradientPhi_Boundary,
					  const SetMatrices<Dimension>& MatricesElem,
					  const MeshNumbering<Dimension>&, int nquad, int num_loc) const;
    ...
    void ComputeNodalValues(const SetMatrices<Dimension>& MatricesElem,
			    ...
			    const Mesh<Dimension>&, int i) const;
    ...
    void ComputeProjectionDof(const SetMatrices<Dimension>& MatricesElem,
			      ...
			      const MeshNumbering<Dimension>&, int n) const;

    template<class T>
    void ComputeProjectionSurfaceDof(const SetMatrices<Dimension>& MatricesElem,
				     Vector<Vector<T> >& feval, Vector<Vector<T> >& res,
				     const MeshNumbering<Dimension>&, int n, int num_loc) const;
    ...
    template<class T>
    void ComputeIntegralSurface(const Vector<typename Dimension::MatrixN_N>& dfjm1,
...
#define MONTJOIE_FILE_FINITE_ELEMENT_H1_HXX
#endif
