#ifndef MONTJOIE_FILE_HEXAHEDRON_HCURL_HP_FIRST_FAMILY_CXX

namespace Montjoie
{
  
  //! default constructor
  HexahedronHcurlHpFirstFamily::HexahedronHcurlHpFirstFamily() : HexahedronReference<2>()
  {
  ...
  size_t HexahedronHcurlHpFirstFamily::GetMemorySize() const
  {
  ...
  //! how to number mesh
  void HexahedronHcurlHpFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
  ...
  void HexahedronHcurlHpFirstFamily
  ...
  //! removing dofs of high-order degree
  /*!
    \param[out] Num new numbers, if -1, the dof is removed
    \param[in] re maximal degree on each edge
    \param[in] rf maximal degree on each face
    \param[in] ri maximal degree in the interior
   */
  /*void HexahedronHcurlHpFirstFamily
  ::CancelHighOrderDofs(IVect& Num, const IVect& re, const IVect& rf, int ri)
  {
    // dofs on edges
    for (int n = 0; n < 12; n++)
      for (int i = re(n); i < order; i++)
        Num(n*order + i) = -1;
    ...
    for (int i = 0; i < order; i++)
      for (int j = 1; j < order; j++)
        {
          int order_dof = max(i, j%order) + 1;
          if (order_dof > rf(0))
            {
              Num(NumDofsY(0, i, j)) = -1;
              Num(NumDofsZ(0, j, i)) = -1;
            }
          
          if (order_dof > rf(1))
            {
              Num(NumDofsX(i, 0, j)) = -1;
              Num(NumDofsZ(j, 0, i)) = -1;
            }
          
          if (order_dof > rf(2))
            {
              Num(NumDofsX(i, j, 0)) = -1;
              Num(NumDofsY(j, i, 0)) = -1;
            }

          if (order_dof > rf(3))
            {
              Num(NumDofsX(i, j, order)) = -1;
              Num(NumDofsY(j, i, order)) = -1;
            }

          if (order_dof > rf(4))
            {
              Num(NumDofsX(i, order, j)) = -1;
              Num(NumDofsZ(j, order, i)) = -1;
            }

          if (order_dof > rf(5))
            {
              Num(NumDofsY(order, i, j)) = -1;
              Num(NumDofsZ(order, j, i)) = -1;
            }
        }
    ...
    for (int i = 0; i < order; i++)
      for (int j = 1; j < order; j++)
        for (int k = 1; k < order; k++)
          {
            int order_dof = max(i, j%order);
            order_dof = max(order_dof, k%order);
            order_dof++;
            if (order_dof > ri)
              {
                Num(NumDofsX(i, j, k)) = -1;
                Num(NumDofsY(j, i, k)) = -1;
                Num(NumDofsZ(j, k, i)) = -1;
              }
          }
          }*/
  ...
  void HexahedronHcurlHpFirstFamily::ConstructFunctions()
  {
    nb_dof_loc = 3*order*(order+1)*(order+1);
    nb_dof_boundaries = 12*order + 12*order*(order-1);
    ...
    ComputeGaussLobatto(points_lob, poids_lob, order);
    ...
    */
    ...
    NumDofsX.Reallocate(order, order+1, order+1);
    NumDofsY.Reallocate(order+1, order, order+1);
    NumDofsZ.Reallocate(order+1, order+1, order);
    NumDofsX.Fill(-1); NumDofsY.Fill(-1); NumDofsZ.Fill(-1);
    // dofs on edges
    for (int i = 0; i < order; i++)
      {
        NumDofsX(i, 0, 0) = i;
        NumDofsY(order, i, 0) = order + i;
        NumDofsX(i, order, 0) = 2*order + i;
        NumDofsY(0, i, 0) = 3*order + i;
        NumDofsZ(0, 0, i) = 4*order + i;
        NumDofsZ(order, 0, i) = 5*order + i;
        NumDofsZ(order, order, i) = 6*order + i;
        NumDofsZ(0, order, i) = 7*order + i;
        NumDofsX(i, 0, order) = 8*order + i;
        NumDofsY(order, i, order) = 9*order + i;
        NumDofsX(i, order, order) = 10*order + i;
        NumDofsY(0, i, order) = 11*order + i;
      }
	...
	for (int j = 0; j < diag; j++)
	  for (int k = 0; k <= diag; k++)
	    if ((i == diag-1) || (j == diag-1) || (k == diag))
	      {
		NumDofsX(k, i+1, j+1) = offset++;
		NumDofsY(i+1, k, j+1) = offset++;
    ...
    for (int i = 0; i <= r; i++)
      {
        ...
        ValBubbleJac(order, i) = points1d(i); DerBubbleJac(order, i) = 1.0;
        for (int j = 0; j < order-1; j++)
          {
  ...
  void HexahedronHcurlHpFirstFamily::ConstructMassMatrix()
  {
    ...
    Matrix<Real_wp> ValBubbleJacNode(order+1, order_geom+1);
    Matrix<Real_wp> DerBubbleJacNode(order+1, order_geom+1);
    VectReal_wp Px, Jx, dJx;
    for (int i = 0; i <= order_geom; i++)
      {
        ...
        ValBubbleJacNode(order, i) = points_nodal1d(i);
        DerBubbleJacNode(0, i) = -1.0;
        DerBubbleJacNode(order, i) = 1.0;
        for (int j = 0; j < order-1; j++)
          {
    ...
    for (int n = 0; n < 6; n++)
      Sh(n).Reallocate(nb_dof_loc, 3*(r+1)*(r+1));
        ...
        for (int j = 0; j <= order; j++)
          {
            ...
            int node = i*(r+1) + j;
            Sh(0).AddInteraction(dof, 3*node, ValLeg(k, r+1));
            Sh(5).AddInteraction(dof, 3*node, ValLeg(k, r+2));
            if (abs(ValBubbleJac(i, r+1)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(1).AddInteraction(dof, 3*node, ValBubbleJac(i, r+1));
              }

            if (abs(ValBubbleJac(i, r+2)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(4).AddInteraction(dof, 3*node, ValBubbleJac(i, r+2));
              }

            if (abs(ValBubbleJac(j, r+1)) > threshold)
              {
                node = k*(r+1) + i;
                Sh(2).AddInteraction(dof, 3*node, ValBubbleJac(j, r+1));
              }

            if (abs(ValBubbleJac(j, r+2)) > threshold)
              {
                node = k*(r+1) + i;
                Sh(3).AddInteraction(dof, 3*node, ValBubbleJac(j, r+2));
              }
            
            dof = NumDofsY(i, k, j);
            node = i*(r+1) + j;
            Sh(1).AddInteraction(dof, 3*node+1, ValLeg(k, r+1));
            Sh(4).AddInteraction(dof, 3*node+1, ValLeg(k, r+2));
            if (abs(ValBubbleJac(i, r+1)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(0).AddInteraction(dof, 3*node+1, ValBubbleJac(i, r+1));
              }

            if (abs(ValBubbleJac(i, r+2)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(5).AddInteraction(dof, 3*node+1, ValBubbleJac(i, r+2));
              }

            if (abs(ValBubbleJac(j, r+1)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(2).AddInteraction(dof, 3*node+1, ValBubbleJac(j, r+1));
              }

            if (abs(ValBubbleJac(j, r+2)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(3).AddInteraction(dof, 3*node+1, ValBubbleJac(j, r+2));
              }

            dof = NumDofsZ(i, j, k);
            node = i*(r+1) + j;
            Sh(2).AddInteraction(dof, 3*node+2, ValLeg(k, r+1));
            Sh(3).AddInteraction(dof, 3*node+2, ValLeg(k, r+2));
            if (abs(ValBubbleJac(i, r+1)) > threshold)
              {
                node = j*(r+1) + k;
                Sh(0).AddInteraction(dof, 3*node+2, ValBubbleJac(i, r+1));
              }

            if (abs(ValBubbleJac(i, r+2)) > threshold)
              {
                node = j*(r+1) + k;
                Sh(5).AddInteraction(dof, 3*node+2, ValBubbleJac(i, r+2));
              }

            if (abs(ValBubbleJac(j, r+1)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(1).AddInteraction(dof, 3*node+2, ValBubbleJac(j, r+1));
              }

            if (abs(ValBubbleJac(j, r+2)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(4).AddInteraction(dof, 3*node+2, ValBubbleJac(j, r+2));
              }            
          }
    ...
    for (int n = 0; n < 6; n++)
      {
        Sh(n).Clear();
        Sh(n).Reallocate(nb_dof_loc, 5*(r+1)*(r+1));
      }
        ...
        for (int j = 0; j <= order; j++)
          {
            int dof = NumDofsX(k, i, j);
            int node = i*(r+1) + j;
            Sh(0).AddInteraction(dof, 5*node, ValLeg(k, r+1));
            Sh(5).AddInteraction(dof, 5*node, ValLeg(k, r+2));
            if (abs(ValBubbleJac(i, r+1)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(1).AddInteraction(dof, 5*node, ValBubbleJac(i, r+1));
              }

            if (abs(ValBubbleJac(i, r+2)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(4).AddInteraction(dof, 5*node, ValBubbleJac(i, r+2));
              }

            if (abs(ValBubbleJac(j, r+1)) > threshold)
              {
                node = k*(r+1) + i;
                Sh(2).AddInteraction(dof, 5*node, ValBubbleJac(j, r+1));
              }

            if (abs(ValBubbleJac(j, r+2)) > threshold)
              {
                node = k*(r+1) + i;
                Sh(3).AddInteraction(dof, 5*node, ValBubbleJac(j, r+2));
              }

            if (abs(DerBubbleJac(i, r+1)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(1).AddInteraction(dof, 5*node+3, DerBubbleJac(i, r+1));
              }

            if (abs(DerBubbleJac(i, r+2)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(4).AddInteraction(dof, 5*node+3, DerBubbleJac(i, r+2));
              }

            if (abs(DerBubbleJac(j, r+1)) > threshold)
              {
                node = k*(r+1) + i;
                Sh(2).AddInteraction(dof, 5*node+3, DerBubbleJac(j, r+1));
              }

            if (abs(DerBubbleJac(j, r+2)) > threshold)
              {
                node = k*(r+1) + i;
                Sh(3).AddInteraction(dof, 5*node+3, DerBubbleJac(j, r+2));
              }
            
            dof = NumDofsY(i, k, j);
            node = i*(r+1) + j;
            Sh(1).AddInteraction(dof, 5*node+1, ValLeg(k, r+1));
            Sh(4).AddInteraction(dof, 5*node+1, ValLeg(k, r+2));
            if (abs(ValBubbleJac(i, r+1)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(0).AddInteraction(dof, 5*node+1, ValBubbleJac(i, r+1));
              }

            if (abs(ValBubbleJac(i, r+2)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(5).AddInteraction(dof, 5*node+1, ValBubbleJac(i, r+2));
              }

            if (abs(ValBubbleJac(j, r+1)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(2).AddInteraction(dof, 5*node+1, ValBubbleJac(j, r+1));
              }

            if (abs(ValBubbleJac(j, r+2)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(3).AddInteraction(dof, 5*node+1, ValBubbleJac(j, r+2));
              }
            
            if (abs(DerBubbleJac(i, r+1)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(0).AddInteraction(dof, 5*node+3, DerBubbleJac(i, r+1));
              }

            if (abs(DerBubbleJac(i, r+2)) > threshold)
              {
                node = k*(r+1) + j;
                Sh(5).AddInteraction(dof, 5*node+3, DerBubbleJac(i, r+2));
              }

            if (abs(DerBubbleJac(j, r+1)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(2).AddInteraction(dof, 5*node+4, DerBubbleJac(j, r+1));
              }

            if (abs(DerBubbleJac(j, r+2)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(3).AddInteraction(dof, 5*node+4, DerBubbleJac(j, r+2));
              }
            
            dof = NumDofsZ(i, j, k);
            node = i*(r+1) + j;
            Sh(2).AddInteraction(dof, 5*node+2, ValLeg(k, r+1));
            Sh(3).AddInteraction(dof, 5*node+2, ValLeg(k, r+2));
            if (abs(ValBubbleJac(i, r+1)) > threshold)
              {
                node = j*(r+1) + k;
                Sh(0).AddInteraction(dof, 5*node+2, ValBubbleJac(i, r+1));
              }

            if (abs(ValBubbleJac(i, r+2)) > threshold)
              {
                node = j*(r+1) + k;
                Sh(5).AddInteraction(dof, 5*node+2, ValBubbleJac(i, r+2));
              }

            if (abs(ValBubbleJac(j, r+1)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(1).AddInteraction(dof, 5*node+2, ValBubbleJac(j, r+1));
              }

            if (abs(ValBubbleJac(j, r+2)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(4).AddInteraction(dof, 5*node+2, ValBubbleJac(j, r+2));
              }            

            if (abs(DerBubbleJac(i, r+1)) > threshold)
              {
                node = j*(r+1) + k;
                Sh(0).AddInteraction(dof, 5*node+4, DerBubbleJac(i, r+1));
              }

            if (abs(DerBubbleJac(i, r+2)) > threshold)
              {
                node = j*(r+1) + k;
                Sh(5).AddInteraction(dof, 5*node+4, DerBubbleJac(i, r+2));
              }

            if (abs(DerBubbleJac(j, r+1)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(1).AddInteraction(dof, 5*node+4, DerBubbleJac(j, r+1));
              }

            if (abs(DerBubbleJac(j, r+2)) > threshold)
              {
                node = i*(r+1) + k;
                Sh(4).AddInteraction(dof, 5*node+4, DerBubbleJac(j, r+2));
              }            
          }
                            ...
                            Matrix<Real_wp>& ValLegendre, Matrix<Real_wp>& ValBubble)
  {
    ...
    Ch3.Reallocate(nb_dof_loc, 3*r*(r+1)*(rq+1));
    ...
    Ch2.Reallocate(3*r*(r+1)*(rq+1), 3*r*(rq+1)*(rq+1));
    for (int k = 0; k <= rq; k++)
      for (int m = 0; m <= rq; m++)
        for (int i = 0; i < r; i++)
          {
                             ...
                             const Matrix<Real_wp>& ValBubble, const Matrix<int>& Num)
  {
    ...
    int r = order;
    int rq = points.GetM() - 1;
    Sh1.Reallocate(3*(r+1)*(r+1), 3*(rq+1)*(r+1));
    Sh2.Reallocate(3*(rq+1)*(r+1), 3*(rq+1)*(rq+1));
    Real_wp vloc, threshold = 1e3*epsilon_machine;
    for (int k = 0; k <= rq; k++)
      {
        for (int i = 0; i <= r; i++)
                                 ...
                                 const Matrix<Real_wp>& ValBubble, const Matrix<int>& Num)
  {
    ...
    int r = order;
    int rq = points.GetM() - 1;
    Sh1.Reallocate(5*(r+1)*(r+1), 6*(rq+1)*(r+1));
    Sh2.Reallocate(6*(rq+1)*(r+1), 3*(rq+1)*(rq+1));
    Real_wp vloc, threshold = 1e3*epsilon_machine;
    for (int k = 0; k <= rq; k++)
      {
        for (int i = 0; i <= r; i++)
  ...
  void HexahedronHcurlHpFirstFamily::ConstructStiffnessMatrix()
  {
    ...
    Matrix<Real_wp> IntPiPj(order, order), IntPiLj(order, order+1), IntLiLj(order+1, order+1);
    Matrix<Real_wp> IntPiDLj(order, order+1), IntLiDLj(order+1, order+1),
      IntDLiDLj(order+1, order+1);
  ...
  */
  template<class Vector1,class Vector2>
  void HexahedronHcurlHpFirstFamily::ComputeValueBoundaryGen(const Vector1& Uh,
  ...
  */
  template<class Vector1,class Vector2>
  void HexahedronHcurlHpFirstFamily::ComputeCurlBoundaryGen(const Vector1& Uh,
                                                            Vector2& Vh, int num_loc) const
  {
  ...
  void HexahedronHcurlHpFirstFamily::SolveMassMatrixGen(Vector1& x) const
  {
  ...
  template<class Vector1>
  void HexahedronHcurlHpFirstFamily::SolveCholeskyGen(const SeldonTranspose& TransA, Vector1& x) const
  {
  ...
  void HexahedronHcurlHpFirstFamily::MltMassMatrixGen(Vector2& x) const
  {
   ...
   */  
  template<class Vector1, class Vector2>
  void HexahedronHcurlHpFirstFamily::ApplyRhGen(const Vector1& Uh, Vector2& Vh) const
  {
   ...
   */
  template<class Vector1, class Vector2>
  void HexahedronHcurlHpFirstFamily::ApplyRhTransposeGen(const Vector1& Uh, Vector2& Vh) const
  {
    Vector2 Uquad(3*nb_points_quadrature_inside);
   ...
   */  
  template<class Vector1, class Vector2>
  void HexahedronHcurlHpFirstFamily::ApplyRhQuadratureGen(const Vector1& Uh, Vector2& Vh) const
  {
    Mlt(rh_loc, Uh, Vh);
  }
   ...
   */
  template<class Vector1, class Vector2>
  void HexahedronHcurlHpFirstFamily::ApplyChGen(const Vector1& Uh, Vector2& Vh) const
  {
   ...
   */
  template<class Vector1, class Vector2>
  void HexahedronHcurlHpFirstFamily::ApplyChTransposeGen(const Vector1& Uh, Vector2& Vh) const
  {
  ...
  */
  void HexahedronHcurlHpFirstFamily::ComputeValuesPhiRef(const R3& point_loc, VectR3& phi) const
  {
  ...
  */
  void HexahedronHcurlHpFirstFamily::ComputeCurlPhiRef(const R3& point_loc, VectR3& res) const
  {
  ...
  */  
  void HexahedronHcurlHpFirstFamily::GetValuePhiOnQuadraturePoint(int kg, VectR3& phi) const
  {
  ...
  */ 
  void HexahedronHcurlHpFirstFamily::GetCurlPhiOnQuadraturePoint(int kg, VectR3& curl_phi) const
  {
    int kx = CoordinateQuad3D(kg, 0);
    int ky = CoordinateQuad3D(kg, 1);
    int kz = CoordinateQuad3D(kg, 2);
...
#define MONTJOIE_FILE_HEXAHEDRON_HCURL_HP_FIRST_FAMILY_CXX
#endif
