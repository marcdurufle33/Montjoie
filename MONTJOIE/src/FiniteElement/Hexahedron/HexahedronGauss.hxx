#ifndef MONTJOIE_FILE_HEXAHEDRON_GAUSS_HXX

namespace Montjoie
{
  
  //! nodal finite element using Gauss-Lobatto points as interpolation points
  //! and Gauss points as quadrature points
  class HexahedronGauss : public HexahedronReference<1>
  {
  protected :
    Matrix<Real_wp> dGL_G, GL_G; //!< \f$ d \varphi^{GL}_i(\xi^G_j) / dx \f$
    Matrix<Real_wp> invGL_G, Gquad_GL;
    
    //! stiffness matrices
    Matrix<Real_wp, General, RowSparse> rh_loc, rh_locX, rh_locY, rh_locZ, rh_weight;
    Matrix<Real_wp, General, RowSparse> rh_weightX, rh_weightY, rh_weightZ;
    Matrix<Real_wp, General, RowSparse> rh_boundary;
    
    //! projection from Gauss-Lobatto points to Gauss points
    Matrix<Real_wp, General, RowSparse> ch1_loc, ch2_loc, ch3_loc;
    Matrix<Real_wp, General, RowSparse> ch3_node, ch1_node, ch2_node;
    Array3D<int> NumDofs3D; Matrix<int> CoordinateDofs, NumDofs2D, CoordinateDofs_quad;
    
    Matrix<Real_wp> GL_Geom, dGL_Geom;
    VectReal_wp invSqrtWeights3d, invWeights3d;
  ...
  public :
    ...
    inline const Array3D<int>& GetNumDofs3D() const { return NumDofs3D; }
...
#define MONTJOIE_FILE_HEXAHEDRON_GAUSS_HXX
#endif
