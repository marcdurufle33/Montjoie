#ifndef MONTJOIE_FILE_HEXAHEDRON_HIERARCHIC_HXX

namespace Montjoie
{
  
  //! Hierarchical finite element with Gauss-Jacobi polynomials
  /*!
    Hierarchical functions are used with Gauss-Jacobi polynomials :
    \psi_i(x) \psi_j(y) \psi_k(z)
    with \psi_0 = 1-x, \psi_1 = x, \psi_i = x (1-x) P_i^{1,1}(2x-1) for i >=2
    with P_i^{alpha, beta} Jacobi polynomials
    Gauss points are used for the quadrature rules
   */
  class HexahedronHierarchic : public HexahedronReference<1>
  {
  protected :
    ...
    Matrix<Real_wp, General, RowSparse> sh1_loc, sh2_loc, rh_loc, const_rh;
    Matrix<Real_wp, Symmetric, RowSymSparse> MhLoc;
    Array3D<int> NumDofs3D;
    Matrix<int> CoordinateDofs, NumDofs2D, CoordinateDofs_quad;
  ...
  public :
    ...
    friend ostream& operator <<(ostream& out, const HexahedronHierarchic& e);
    
  };

} // namespace Montjoie

#define MONTJOIE_FILE_HEXAHEDRON_HIERARCHIC_HXX
#endif
