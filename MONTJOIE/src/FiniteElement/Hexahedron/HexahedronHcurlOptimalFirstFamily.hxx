#ifndef MONTJOIE_FILE_HEXAHEDRON_HCURL_OPTIMAL_FIRST_FAMILY_HXX
...
namespace Montjoie
{
  ...
  /*!
    ...
    phi_i^Gint(x) phi_j^GL(y) phi_k^GL(z)  e_x
    phi_j^GL(x) phi_i^Gint(y) phi_k^GL(z)  e_y
    phi_j^GL(x) phi_k^GL(y) phi_i^Gint(z)  e_z
    with i <= r-1, j <= r+1, k <= r+1
    phi_i^Gint are Lagrange functions based on interior Gauss-Lobatto points
    Gauss-Lobatto points are used for integration so that Ch is very sparse
   */
  class HexahedronHcurlOptimalFirstFamily : public HexahedronReference<2>
  {
  protected :
    ...
    Matrix<Real_wp> dGL_GL, stiff1d, GLint_GL, GL_GX, Geom_GL;
    VectR2 ValGaussExt;
    ...
    Matrix<Real_wp> WeightsMassGL, invWeightsMassGL, invSqrtWeightsMassGL;
  ...
  protected:
    ...
    void ApplyShGen(const T0& alpha, int num_loc, const Vector1& feval,
		    Vector2& res, int r = 0) const;
    
    template<class Vector1, class Vector2>
    void ApplyShTransposeGen(int num_loc, const Vector1& res, Vector2& feval, int r = 0) const;
...
#define MONTJOIE_FILE_HEXAHEDRON_HCURL_OPTIMAL_FIRST_FAMILY_HXX
#endif
