#ifndef MONTJOIE_FILE_HEXAHEDRON_HDIV_OPTIMAL_HP_FIRST_FAMILY_CXX

namespace Montjoie
{
  
  //! default constructor
  HexahedronHdivOptimalHpFirstFamily::HexahedronHdivOptimalHpFirstFamily()
    : HexahedronReference<3>()
  {
    this->Fb_geom.quadrature_equal_nodal = false;
    this->Fb_geom.dof_equal_nodal = false;
    this->Fb_geom.dof_equal_quadrature = false;
    type_basis = OPTIMAL;
    //type_basis = SUPER_OPTIMAL;
  }
  

  //! how to number mesh
  void HexahedronHdivOptimalHpFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
    ...
    if (type_basis == OPTIMAL)
      ...
      nmap.SetNbDofQuadrangle(order, (order+1)*(order+1)-1);
  ...
  size_t HexahedronHdivOptimalHpFirstFamily::GetMemorySize() const
  {
  ...
  void HexahedronHdivOptimalHpFirstFamily::ConstructFiniteElement(int r, int rgeom,
								  ...
								  int type_surf_tri, int type_surf_quad, int gauss_z)
  {
    ...
    //this->mass_matrix.Write("MhHexDiv"+to_str(this->order)+".dat");
    //this->const_div_matrix.Write("KhHexDir"+to_str(this->order)+".dat");
  ...
  void HexahedronHdivOptimalHpFirstFamily::ConstructFunctions()
  {
    nb_dof_quad = (order+1)*(order+1);
    nb_dof_tri = 0;
    nb_dof_loc = 3*(order+1)*(order+1)*(order+3);
    nb_dof_boundaries = 6*(order+1)*(order+1);    
    ...
    GetJacobiPolynomial(JacobiPolynom, order+1, Real_wp(1), Real_wp(1));
    
    NumDofsX.Reallocate(order+3, order+1, order+1);
    NumDofsY.Reallocate(order+1, order+3, order+1);
    NumDofsZ.Reallocate(order+1, order+1, order+3);
    ...
    for (int i = 0; i <= order; i++)
	  ...
	  NumDofsZ(i, j, order+2) = 3*N + node;
	  NumDofsY(i, order+2, j) = 4*N + node;
	  NumDofsX(order+2, i, j) = 5*N + node;
        }
    ...
    int r = order_quad;
    ValLeg.Reallocate(order+1, r+3);
    ValBubbleJac.Reallocate(order+3, r+3);
    DerBubbleJac.Reallocate(order+3, r+3);
    ValBubbleJac.Fill(0);
    DerBubbleJac.Fill(0);
    ...
    for (int i = 0; i <= r; i++)
      {
        EvaluateJacobiPolynomial(LegendrePolynom, order, 2.0*points1d(i)-1.0, Px);
        ...
        ValBubbleJac(order+2, i) = points1d(i); DerBubbleJac(order+2, i) = 1.0;
        for (int j = 0; j <= order; j++)
          {
            ValBubbleJac(j+1, i) = Jx(j)*points1d(i)*(1.0-points1d(i));
            DerBubbleJac(j+1, i) = (1.0 - 2.0*points1d(i))*Jx(j)
              + 2.0*dJx(j)*points1d(i)*(1.0-points1d(i));
          }        
      }
    
    // values on extremities are stored at the end
    EvaluateJacobiPolynomial(LegendrePolynom, order, Real_wp(-1), Px);
    for (int j = 0; j <= order; j++)
      ValLeg(j, r+1) = Px(j);
    ...
    ValBubbleJac(0, r+1) = 1.0; ValBubbleJac(order+2, r+1) = 0.0;
    DerBubbleJac(0, r+1) = -1.0; DerBubbleJac(order+2, r+1) = 1.0;
    EvaluateJacobiPolynomial(JacobiPolynom, order+1, Real_wp(-1), Jx);
    for (int j = 0; j <= order; j++)
      DerBubbleJac(j+1, r+1) = Jx(j);
    
    ValBubbleJac(0, r+2) = 0.0; ValBubbleJac(order+2, r+2) = 1.0;
    DerBubbleJac(0, r+2) = -1.0; DerBubbleJac(order+2, r+2) = 1.0;
    EvaluateJacobiPolynomial(JacobiPolynom, order+1, Real_wp(1), Jx);
    for (int j = 0; j <= order; j++)
    ...
    const Matrix<int>& NumQuad2D = this->GetNumQuad2D();
    r = NumQuad2D.GetM()-1;
    Matrix<Real_wp> ValLegSurf(order+1, r+1);
    for (int i = 0; i <= r; i++)
      {
	Real_wp xi = this->Points2D_quad()(NumQuad2D(i, 0))(0);
        EvaluateJacobiPolynomial(LegendrePolynom, order, 2.0*xi-1.0, Px);
        for (int j = 0; j <= order; j++)
          ValLegSurf(j, i) = Px(j);
      }
    ...
    if (type_basis == OPTIMAL)
      {
        ValuePhi2D.Reallocate((order+1)*(order+1), this->Points2D_quad().GetM());
        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            for (int k1 = 0; k1 <= r; k1++)
              for (int k2 = 0; k2 <= r; k2++)
                ValuePhi2D(i*(order+1) + j, NumQuad2D(k1, k2))
                  = ValLegSurf(i, k1)*ValLegSurf(j, k2);    
      }
    else
      {
        int num = 0;
        ValuePhi2D.Reallocate((order+1)*(order+1)-1, this->Points2D_quad().GetM());
        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              {
                for (int k1 = 0; k1 <= r; k1++)
                  for (int k2 = 0; k2 <= r; k2++)
                    ValuePhi2D(num, NumQuad2D(k1, k2))
                      = ValLegSurf(i, k1)*ValLegSurf(j, k2);    
  ...
  void HexahedronHdivOptimalHpFirstFamily::ConstructMassMatrix()
  {
  }
  
  
  //! constructing stiffness matrix
  void HexahedronHdivOptimalHpFirstFamily::ConstructStiffnessMatrix()
  {
  ...
  */
  void HexahedronHdivOptimalHpFirstFamily
  ::ComputeValuesPhiRef(const R3& point_loc, VectR3& phi) const
  {
    ...
    EvaluateJacobiPolynomial(LegendrePolynom, order, 2*x-1, Px);
    EvaluateJacobiPolynomial(LegendrePolynom, order, 2*y-1, Py);
    EvaluateJacobiPolynomial(LegendrePolynom, order, 2*z-1, Pz);
    ...
    Jx2.Reallocate(order+3);
    Jy2.Reallocate(order+3);
    Jz2.Reallocate(order+3);
    
    Jx2(0) = 1.0-x; Jx2(order+2) = x;
    Jy2(0) = 1.0-y; Jy2(order+2) = y;
    Jz2(0) = 1.0-z; Jz2(order+2) = z;
    for (int i = 0; i <= order; i++)
      {
    ...
    if (type_basis == SUPER_OPTIMAL)
      {
        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              phi(node++).Init((1.0-x)*Py(i)*Pz(j), 0.0, 0.0);
        
        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              phi(node++).Init(0.0, -(1.0-y)*Px(i)*Pz(j), 0.0);

        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              phi(node++).Init(0.0, 0.0, (1.0-z)*Px(i)*Py(j));

        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              phi(node++).Init(0.0, 0.0, z*Px(i)*Py(j));

        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              phi(node++).Init(0.0, -y*Px(i)*Pz(j), 0.0);

        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            if ((i != order) || (j != order))
              phi(node++).Init(x*Py(i)*Pz(j), 0.0, 0.0);
          ...
          for (int j = 0; j <= order; j++)
            for (int k = 0; k <= order; k++)
              if ( (i == order) || (j != order) || (k != order))
                {
                  phi(node++).Init(x*(1.0-x)*Jx(i)*Py(j)*Pz(k), 0.0, 0.0);
                  phi(node++).Init(0.0, y*(1.0-y)*Jy(i)*Px(j)*Pz(k), 0.0);
                  phi(node++).Init(0.0, 0.0, z*(1.0-z)*Jz(i)*Px(j)*Py(k));
                }
        
        phi(node++).Init(x*(1.0-x)*Jx(order-1)*Py(order)*Pz(order),
                         y*(1.0-y)*Jy(order-1)*Px(order)*Pz(order),
                         z*(1.0-z)*Jz(order-1)*Px(order)*Py(order)); 
  ...
  */
  ...
  ::ComputeDivPhiRef(const R3& point_loc, VectReal_wp& res) const
  {
    ...
    dJx2.Reallocate(order+3);
    dJy2.Reallocate(order+3);
    dJz2.Reallocate(order+3);
    
    dJx2(0) = -1.0; dJx2(order+2) = 1.0;
    dJy2(0) = -1.0; dJy2(order+2) = 1.0;
    dJz2(0) = -1.0; dJz2(order+2) = 1.0;
    for (int i = 0; i <= order; i++)
      {
        ...
        for (int i = 0; i <= order; i++)
          for (int j = 0; j <= order; j++)
            for (int k = 0; k <= order; k++)
              if ( (i == order) || (j != order) || (k != order))
                {
                  res(node++) = ( (1.0-2.0*x)*Jx(i) + 2.0*x*(1.0-x)*dJx(i))*Py(j)*Pz(k);
                  res(node++) = ( (1.0-2.0*y)*Jy(i) + 2.0*y*(1.0-y)*dJy(i))*Px(j)*Pz(k);
                  res(node++) = ( (1.0-2.0*z)*Jz(i) + 2.0*z*(1.0-z)*dJz(i))*Px(j)*Py(k);
                }
        
        res(node++) = ( (1.0-2.0*x)*Jx(order-1) + 2.0*x*(1.0-x)*dJx(order-1))*Py(order)*Pz(order)
          + ( (1.0-2.0*y)*Jy(order-1) + 2.0*y*(1.0-y)*dJy(order-1))*Px(order)*Pz(order)
          + ( (1.0-2.0*z)*Jz(order-1) + 2.0*z*(1.0-z)*dJz(order-1))*Px(order)*Py(order); 
...
#define MONTJOIE_FILE_HEXAHEDRON_HDIV_OPTIMAL_HP_FIRST_FAMILY_CXX
#endif
