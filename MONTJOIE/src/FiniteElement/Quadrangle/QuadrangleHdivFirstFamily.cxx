#ifndef MONTJOIE_FILE_QUADRANGLE_HDIV_FIRST_FAMILY_CXX

namespace Montjoie
{
  //! default constructor
  QuadrangleHdivFirstFamily::QuadrangleHdivFirstFamily() : QuadrangleReference<3>()
  {
  ...
  //! how to number mesh
  void QuadrangleHdivFirstFamily::ConstructNumberMap(NumberMap& nmap, int dg) const
  {
  ...
  size_t QuadrangleHdivFirstFamily::GetMemorySize() const
  {
  ...
  void QuadrangleHdivFirstFamily
			   ...
			   int rsurf, int type_surf)
  {
    ...
    QuadrangleReference<3>::ConstructFiniteElement(r, rgeom, r, type_quad);
    
    lob_basis.ConstructQuadrature(order-1, Globatto<Real_wp>::QUADRATURE_GAUSS);
    lob_GL.ConstructQuadrature(order, Globatto<Real_wp>::QUADRATURE_LOBATTO);
  ...
  void QuadrangleHdivFirstFamily::ConstructFunctions()
  {
    EdgesDof.Reallocate(order, 4);
    NumDofs_X.Reallocate(order+1, order);
    NumDofs_Y.Reallocate(order, order+1);
	...
	NumDofs_Y(order-1-i, order) = EdgesDof(i, 2);
    ...
    for (int i = 0; i < order; i++)
      for (int j = 0; j <= order; j++)
	{
	  points_dof2d(NumDofs_Y(i,j)).Init(lob_basis.Points(i), lob_GL.Points(j));
	  points_dof2d(NumDofs_X(j,i)).Init(lob_GL.Points(j), lob_basis.Points(i));
  ...
  void QuadrangleHdivFirstFamily::ConstructStiffnessMatrix()
  {
  }
  
  
  //! computation of mass matrix
  void QuadrangleHdivFirstFamily::ConstructMassMatrix()
  {
    ...
    for (int i = 0; i <= order; i++)
      for (int j = 0; j <= order; j++)
	GL_Gexact(i, j) = lob_GL.EvaluatePhi(i, gauss.Points(j));
  }
  ...
  */
  void QuadrangleHdivFirstFamily::ComputeValuesPhiRef(const R2& point_loc, VectR2& res) const
  {
    ...
    for (int i = 0; i <= order; i++)
      {
	phiGL_x(i) = lob_GL.EvaluatePhi(i, point_loc(0));
	phiGL_y(i) = lob_GL.EvaluatePhi(i, point_loc(1));
      }
  ...
  */
  void QuadrangleHdivFirstFamily::ComputeDivPhiRef(const R2& point_loc, VectReal_wp& res) const
  {
    res.Reallocate(nb_dof_loc);
    res.Fill(0);
    ...
    for (int i = 0; i <= order; i++)
      {
	dphiGL_x(i) = lob_GL.EvaluatePhiGrad(i, point_loc(0));
	dphiGL_y(i) = lob_GL.EvaluatePhiGrad(i, point_loc(1));
      }
...
#define MONTJOIE_FILE_QUADRANGLE_HDIV_FIRST_FAMILY_CXX
#endif
