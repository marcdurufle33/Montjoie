...
namespace Montjoie
{
  ...
  ::SetIndexToCompute(VarGalbrunIndex_Base<Dimension>& var)
  {
    ...
    var.compute_source_momentum = false; //AJOUT NATHAN
  }
  ...
  */
		    ...
		    Vector<bool>& compute_grad, Vector<bool>& compute_hess)
  {
    ...
    nb = 0;
    Vector<bool> RefUsed(ref_v0.GetM());
    RefUsed.Fill(false);
    ...
    if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
      cout << endl;
    
    for (int i = 0; i < ref_v0.GetM(); i++)
      {
        TinyVector<bool, Dimension::dim_N> var_m;
        var_m.Fill(!RefUsed(i));
	int nb0 = nb;
	...
	for (int j = nb0; j < nb; j++)
	  {
	    ...
	    if (compute_hessian_flow)
	      compute_hess(j) = true;
            ...
            if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
              { cout << "The velocity M" << num_component(j) << " for reference " << i << " is variable" << endl; var_m(j-nb0) = true; }
	  }

        for (int m = 0; m < Dimension::dim_N; m++)
          if (!var_m(m))
            if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
              cout << "M" << m << " for reference " << i << " = " << ref_v0(i).GetConstant()(m) << endl;
	...
	for (int j = nb0; j < nb; j++)
	  {
	    ...
	    if (compute_grad_sigma)
	      compute_grad(j) = true;
            ...
            if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
              { cout << "The damping sigma for reference " << i << " is variable" << endl; variable = true; }
	  }

        if (!variable)
          if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
            cout << "Sigma for reference " << i << " = " << ref_sigma(i).GetConstant() << endl;
	...
	for (int j = nb0; j < nb; j++)
	  {
	    ...
	    if (compute_grad_rho || compute_grad_gamma)
	      compute_grad(j) = true;
            ...
            if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
              { cout << "The density rho for reference " << i << " is variable" << endl; variable = true; }
	  }

        if (!variable)
          if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
            cout << "Density for reference " << i << " = " << ref_rho0(i).GetConstant() << endl;
	...
	for (int j = nb0; j < nb; j++)
	  {
	    ...
	    if (compute_grad_c0 || compute_grad_gamma)
	      compute_grad(j) = true;
            ...
            if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
              { cout << "The velocity c0 for reference " << i << " is variable" << endl; variable = true; }
	  }

        if (!variable)
          if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
            cout << "Density for reference " << i << " = " << ref_c0(i).GetConstant() << endl;
	...
	for (int j = nb0; j < nb; j++)
	  {
	    ...
	    if (compute_hessian_p0)
	      compute_hess(j) = true;
            ...
            if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
              {cout << "The pressure for reference " << i << " p0 is variable" << endl; variable = true; }
	  }

        if (!variable)
          if ((print_level >= 2) && (var_problem.GetRankProcMode() == 0))
            cout << "Pressure for reference " << i << " = " << ref_p0(i).GetConstant() << endl;
  ...
  {
    //cout << "JE PASSE BIEN LA " << endl;
    DISP(compute_grad_flow);
		...
		for (int j = 0; j < nb_bounds; j++)
		  {
		    ...
		    int nb_pts_face = mesh_num.GetNbPointsQuadratureBoundary(num_face);
	...
	//eval_f0(i).WriteText("eval_f0_"+std::to_string(i)+".dat");
      }
	...
	for(int j=0; j<Fb.GetNbDof(); j++){
	  ...
	  //fy(numDof) = eval_f0(i)(j)(1);
	}
      }
      var_problem.GetOutputProblem().WriteOutputFile(f,"source_f",2);
      //var_problem.GetOutputProblem().WriteOutputFile(fx,"source_fx",1);
      //var_problem.GetOutputProblem().WriteOutputFile(fy,"source_fy",1);
