...
C                        = 2  RETURNS
C                             AI=EXP(ZTA)*AI(Z)       ON ID=0 OR
C                             AI=EXP(ZTA)*DAI(Z)/DZ   ON ID=1 WHERE
C                             ZTA=(2/3)*Z*SQRT(Z)
C
C         OUTPUT
...
C-----------------------------------------------------------------------
   ...
   40   CONTINUE
        S2 = CS1
        IF (X+X.LT.ELIM) S2 = S2 + P1*CS2*EXP(-Z-Z)
...
C
C               H(1,-FNU,Z) = H(1,FNU,Z)*EXP( PI*FNU*I)
C               H(2,-FNU,Z) = H(2,FNU,Z)*EXP(-PI*FNU*I)
C                         I**2=-1
C
C         CAN BE USED.
C
...
C         KODE=2, A SCALING OPTION EXP(-AXZTA)*BI(Z) OR EXP(-AXZTA)*
C         DBI(Z)/DZ IS PROVIDED TO REMOVE THE EXPONENTIAL BEHAVIOR IN
C         BOTH THE LEFT AND RIGHT HALF PLANES WHERE
C         ZTA=(2/3)*Z*SQRT(Z)=DCMPLX(XZTA,YZTA) AND AXZTA=DABS(XZTA).
C         DEFINITIONS AND NOTATION ARE FOUND IN THE NBS HANDBOOK OF
C         MATHEMATICAL FUNCTIONS (REF. 1).
C
C         INPUT
...
C                        = 2  RETURNS
C                             BI=EXP(-AXZTA)*BI(Z)     ON ID=0 OR
C                             BI=EXP(-AXZTA)*DBI(Z)/DZ ON ID=1 WHERE
C                             ZTA=(2/3)*Z*SQRT(Z)=DCMPLX(XZTA,YZTA)
...
C-----------------------------------------------------------------------
C     THE DIVISION EXP(PT)/(SUM+P2) IS ALTERED TO AVOID OVERFLOW
...
C
C         ZETA1=0.5*FNU*LOG((1+W)/(1-W)), ZETA2=FNU*W FOR SCALING
...
C***END PROLOGUE  CUNHJ
     ...
     * BSUMI, TEST, TSTR, TSTI, AC, DZABS, D1MACH
...
C***END PROLOGUE  CUNIK
     ...
     * T2, ZETA1, ZETA2, ZN, ZR
      REAL*8 AC,C,FNU,RFN, TEST, TOL, TSTR, TSTI, D1MACH
...
C-----------------------------------------------------------------------
   ...
   80 CONTINUE
      ...
      CZ = CZ - DCMPLX(0.25D0,0.0D0)*LOG(ARG) - DCMPLX(AIC,0.0D0)
   90 CONTINUE
      AX = DEXP(RCZ)/TOL
      AY = DIMAG(CZ)
...
C
      ...
      REAL*8 X, B, V, EM, EM9, C13, C23, EM2, E12, TB, TB2
      REAL*8 X0, X1, AN2, AN3, AN4, AN5, AN6
      REAL*8 S2, S21, S22, S23, DELX, XX, ZL, T, TS, ETA, ZN
      REAL*8 TEMP, TEMP2, RETA
      INTEGER NB, NERROR, L, M, NBITS, I, INIT, NITER
      INTEGER NN
      REAL*8 J, WAPR
...
C
C COMMON/WAPCOM/NBITS
C DATA INIT,NITER/0,1/
C    DATA NBITS/23/
C
      IF(INIT.EQ.0) THEN
         INIT=1
...
C
     ...
     *           AN6+AN5))+AN3)))-1.D0
         ...
         ELSE IF(XX.LE.2.D1) THEN
            RETA=S2*SQRT(1.D0-XX/EM)
            AN2=4.612634277343749D0*SQRT(SQRT(RETA+
     *           1.09556884765625D0))
            WAPR=RETA/(1.D0+RETA/(3.D0+(S21*AN2+S22)*RETA/
     *           (S23*(AN2+RETA))))-1.D0
         ELSE
            ZL=LOG(XX)
            WAPR=LOG(XX/LOG(XX/ZL**EXP(-1.124491989777808D0/
...
C
     ...
     *           TS*127.0471381349219D0))*TS)
         ELSE
            ZL=LOG(-XX)
            ETA=2.D0-EM2*XX
            WAPR=LOG(XX/LOG(-XX/((1.D0-.5043921323068457D0*
     *           (ZL+1.D0))*(SQRT(ETA)+ETA/3.D0)+1.D0)))
         ENDIF
      ENDIF
      DO  I=1,NITER
         ZN=LOG(XX/WAPR)-WAPR
...
C***********************************************************
      SUBROUTINE LAMBERT(X, Y)
      
      IMPLICIT NONE
      
      REAL*8 X, Y
      REAL*8  ERROR
      INTEGER L,M, NB,NERROR
      REAL*8 WAPR
      EXTERNAL WAPR
      ...
      NB = 0
      M = 0
      Y  =  WAPR(X,NB,NERROR,L,M) 
      
      
      IF( NERROR .eq. 1) THEN
         
         WRITE(6,*) ' X out of range in LAMBERT(X)'
         WRITE(6,*) 'X=',X, ' LAMBERT= ',Y
         STOP
      ...
      ELSEIF( M .eq. 1 ) THEN
         ...
         IF( X .ne. 0 ) THEN
            ERROR = abs( ( Y* exp(Y) - X)/X )
            WRITE(6,*) ' abs( LAMBERT* exp(LAMBERT)- X)/X = ',ERROR
            
         ELSE
            ERROR = abs(  Y* exp(Y) )
            WRITE(6,*) ' abs( LAMBERT* exp(LAMBERT) ) ', ERROR
         ENDIF
