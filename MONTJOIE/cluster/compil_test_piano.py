...
target_compil = ["multistringStiffNL", "multistringStiffLIN", "multistringStiff", "multistringNL", "multistringLIN", \
         "PianoStiffNL", "PianoStiffLIN", "PianoStiff", "PianoNL","PianoLIN", \
         "StringImpedanceStiffNL", "soundboard", "static_elastic", \
         "StringSoundboardStiffNL", "StringSoundboardLIN", "StringSoundboardStiff", "StringSoundboardNL", "StringSoundboardLIN"]
...
def SetSimulation(index):
    return "cd "+current_dir+"/..; source ~/.bash_profile; make " + target_compil[index] + " >& cluster/errMake" + str(index)

# tant que toutes les compilations ne sont pas terminees
success_compil = True
nom_fichier = "/home-ext/chabassi/log_compil_test.txt"
    ...
    if all_launched or quota_reached or network_busy:
        ...
        for i in range(nb_simul_ef):
            if (simul_done[i] == 1 \
                ...
                simul_done[i] = 2
                simul_machines[simul_machines_index[i]] -= 1
